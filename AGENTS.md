...
- **중복 금지.** 같은 내용이 SKILL.md 와 references 양쪽에 있으면 안 된다 — 옮겼으면
  본문에서 지운다.
- **판정 근거로 부르는 절은 `assets/` 템플릿에 실제로 만든다.** 문서가 `` `### 불변식` ``
  처럼 절 이름을 근거로 지목하는데 템플릿에 그 절이 없으면 그 근거는 영원히 비어 있고,
  실패가 무음이다(`## 판정 기준` 계열 6건이 그렇게 떠 있었다). 우리 템플릿의 슬롯이 아닌
  절(PR 본문 섹션, 스크립트가 생성하는 절)은 검사기의 `SECTION_ALLOWLIST` 에 **이유와
  함께** 등재한다.
...
파일의 SKILL.md 참조, 스킬 루트 고아 .md 부재, **백틱으로 인용한 절 이름의 실존**을
검사한다. FAIL 이면 고치고 나서 커밋한다. 검사 로직을 고쳤으면
`python3 tools/check_skill_docs.py --selftest` 도 돌린다.

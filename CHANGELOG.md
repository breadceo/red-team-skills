...
- **3단 폴백**: ① json 태그 펜스(1순위 유지) → ② 언어 태그 없는 펜스 → ③ bare JSON.
  ...
  파싱에 실패한 span·`[` 배열 컨테이너는 통째로 건너뛴다(문자열·escape·여닫이
  종류(stack) 인식 반복문 스캐너) — 깊은 중첩의 비선형 폭증, 깨진 외곽 객체·미닫힌
  배열의 내부 조각(짝 안 맞는 closer 포함) 오인을 함께 없앤다. 전 경로가 입력 길이에
  선형이다.
...
- 테스트: `test_extract_json.py` 신규(41건 — 기존 경로 회귀 방지, 실측 bare 형태,

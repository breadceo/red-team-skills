...
- **3단 폴백**: ① json 태그 펜스(기존 동작, 1순위 유지) → ② 언어 태그 없는 펜스 →
  ③ 문서 끝에서 역방향으로 bare JSON(`raw_decode` — 정규식 균형 카운팅은 문자열 안
  중괄호에 어긋난다). 세 경로 모두 같은 수락 조건(dict + `findings` 키)을 통과해야
  하고 뒤에서부터 찾으므로, 서술 중간 예시 JSON 보다 진짜 판정이 먼저 걸린다.
- **PARSE-FAIL 진단 한 줄**: raw 크기·엔진 에러 수를 병기해 실측 3유형(모델 용량 /
  실행 환경 / 파서 미스)이 로그에서 바로 갈린다 — 사람이 raw 를 열어 재지 않아도 된다.
- 테스트: `test_extract_json.py` 신규(14건 — 기존 경로 회귀 방지, 실측 bare 형태,
  마지막 판정 우선, 수락 조건, 문자열 안 중괄호).

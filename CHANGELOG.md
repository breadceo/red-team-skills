...
- **3단 폴백**: ① json 태그 펜스(1순위 유지) → ② 언어 태그 없는 펜스 → ③ bare JSON.
  ...
  패스라 lazy 정규식의 O(n²) 재탐색이 없다. bare 후보는 **줄 시작(선행 공백 허용) opener 뿐**이다 —
  산문에 인라인으로 파묻힌 중괄호(미종결 인용 안 조각 포함)는 후보도 차단자도
  아니며, 성공한 객체의 끝으로 점프해 중첩 내부 객체가 판정으로 오인되지 않고,
...
- 테스트: `test_extract_json.py` 신규(61건 — 기존 경로 회귀 방지, 실측 bare 형태,

...
리뷰어 탈출 결함 실측에서 나온 처방 15건을 순차로 닫았다(#52~#66). 그 뒤 **보강이 실제로
효과가 있었는지 같은 배선으로 재봤고**(#82), 결과가 전제와 달라 그것도 함께 적는다.

### 📊 근거 — 게이트가 GO 를 낸 뒤 리뷰어가 잡은 결함 186건

`~/.red-team/` 의 pr-triage 기록에서 다른 리뷰어(hermes·aws-security-agent·사람)가
`real-defect` 로 확정한 코멘트 127건을 본문까지 받아 **186 finding** 으로 쪼개 분류했다. 각
finding 을 그때의 5축 프롬프트와 대조해 `covered`(축이 지시했는데 놓침) 66 · `partial`(인접
문장은 있으나 그 표면을 열거하지 않음) 67 · `none`(어느 축도 안 다룸) 53 으로 갈랐다.

세는 기준을 두 번 고쳤다. **(1)** 코멘트 `created_at` 과 라운드 완료 시각을 대조하지 않으면
게이트가 돌기도 전에 달린 코멘트가 섞인다 — 186건 중 42건(23%)이 그랬다. **(2)** 탈출 건수는
그 PR 이 받은 코멘트 수와 corr 0.72 다. 노출량으로 정규화하면 라운드 수·diff 크기와의 상관이
+0.68·+0.47 에서 **-0.22·-0.16 으로 뒤집힌다** — 「라운드를 더 돌면 더 샌다」는 교란이었다.

### ✨ red-team — `a-code` 「어떻게 찾나」에 방향 네 개가 생겼다 (#53 · #54 · #57)

기존에는 **호출부 전수**(「이 심볼을 누가 쓰나」)만 있었다. 세 방향이 붙었다.

- **형제 전수**(#53) — 「같은 결과에 도달하는 다른 길이 무엇인가」. 결과 쪽에서 역방향으로
  같은 싱크의 형제 필드 · 같은 피호출자의 형제 호출부 · 같은 계약의 형제 구현(모노레포의 다른
  앱 포함) · 다른 배포 라인을 나열한다. 일괄 치환의 사본 대조와 목적을 잃은 잔존 가드도 여기다
- **소비처 계약**(#54) — 옆·뒤를 봤으면 남은 방향은 **앞**이다. 값이 도달하는 DTO 제약 · DB
  컬럼 폭 · UI 가 약속한 상한 · 하류 수치 연산에 셋을 대입한다(같은 수인가 · 같은 단위로
  세는가 · 통과값이 `Infinity`·`NaN` 이 되어 비교를 흡수하는가)
- **매처 입력집합**(#57) — 남은 것은 **입구**다. 5번 (c)가 「막겠다고 선언한 것을 막는가」라면
  이쪽은 「통과시키는 집합이 의도한 집합인가」다. 넓어진 쪽·좁은 쪽을 다 대입하고, 열거는
  패턴이 아니라 **입력 쪽에서** 한다

### ✨ red-team — 각 축의 열거 대상을 넓혔다 (#55 · #56 · #58 · #59 · #60 · #61 · #62 · #63)

- **`a-code` #3 — 서술도 대조 대상이다**(#55). 주석·docblock·표·PR 본문이 서술하는 동작·계약·
  값·수치를 코드에 대입한다. 전칭 단정(「전부」·「하나도」)은 형제 요소 전수 대입, 표는 행마다
  grep. `파일:줄` 앵커가 **같은 diff 로 밀렸는지**, 배포물(`dist`·`node_modules`)·버전 미고정
  의존을 라인번호로 인용했는지, JSDoc 이 심볼에서 고아가 됐는지까지 본다
- **`a-code` #4 — 올라간다는 것과 옳게 적재된다는 것은 다르다**(#56). 레벨(정상 종료를 `error`
  로 적재하는가) · 버킷(정상 종료가 실패 분자에 섞이는가) · 귀속(`setUser` 가 발화보다 앞서는가,
  로그아웃 경로마다 해제가 짝이 맞는가) · 그룹핑 키와 문맥 필드. **문맥 필드 유실은 층 나열로
  보이지 않는다** — 이벤트는 그대로 올라간다
- **`a-code` #5 — 막는가에 더해 도는가**(#58). 새 테스트·lint 규칙을 부르는 단계가 CI 에 있는지,
  이 변경이 기존 게이트(tsc·lint)를 깨뜨리는지, 그 장치가 코드에 안 적힌 암묵 전제(미선언 전이
  의존성 import·수동 관리 목록)에 기대는지. 함께 `_common.md` 의 「테스트가 없다는 것 자체는
  결함이 아니다」에 **좁은 예외**를 달았다 — ① 이 변경이 스스로 「조용히 깨져 있었다」고
  진단했거나 여러 파일 공동 불변식이고 ② 그것을 고정하는 단언이 diff 에 없을 때. **두 조건 중
  하나라도 확인하지 못하면 지적하지 않는다**
- **`a-code` 실패 경로 — 범위와 수명**(#60). 배치 루프에서 한 항목의 **동기 throw** 가 어디까지
  중단시키는지, 타임아웃·폴백·이탈로 먼저 진행한 뒤에도 살아 있는 요청·타이머·구독. *기다림을
  끝내는 것과 그 일을 멈추는 것은 다르다 — `Promise.race`·`setTimeout` 은 진 쪽을 취소하지 않는다*
- **`b1` — 열은 고정이 아니다**(#59 · #52). 6개 열은 바닥이고, 조건이 걸리는 surface 에만 열을
  **더 세운다**: 캐시가 이전 값을 보존하는 쿼리 → 「성공 후 재조회 실패」 · 한 분기가 사유를 둘
  이상 받으면 → 사유마다 한 열 · 외부 값이 분기를 고르면 → 「예상 밖·부분값」 · **비동기 판정이
  진행 중인데 입력이 바뀔 수 있으면 → 「판정 중 입력 교체」**(#52). 새 분기 키를 도입하면 표에
  축이 하나 는다
- **`b2` — 사용자가 누르는 것만 컨트롤이 아니다**(#61). 뒤로가기·popstate·라우트 전환과 자동
  실행 경로를 목록에 올리고, 컨트롤마다 넷을 대입한다(해소 시점이 오는가 · 두 번 발화하는가 ·
  막아야 할 때 막는가 · 이탈한 뒤 도착하는 응답). *가드 없는 컨트롤은 죽은 컨트롤의 반대다 —
  막아야 할 때 동작한다*
- **`b4` — 없음의 반대 방향과 순서**(#62). 이 변경이 손댄 판정식에 참여하는 **기존** nullable,
  공백 문자열·빈 배열처럼 의미상 없음인데 truthy 인 값, 삭제·초기화가 이후 읽기보다 먼저 도는 순서
- **`b3` — 색 밖으로**(#63). 상속으로 전파되는 CSS 속성을 컨테이너에서 바꿨으면 하위 직접 선언도
  전수, 호출부가 표준 유틸로 되돌릴 때 같은 병합 그룹인지. 이 축은 498 findings 중 12건만 냈지만
  **전부 P1** 이라 물량을 늘리지 않는 쪽으로 폭을 좁혔다

### ✨ red-team — GO 후보 라운드에서 core 축을 재샘플링한다 (#66)

같은 diff·같은 컨텍스트로 6라운드를 돌렸더니 verdict 가 **GO 2 / NO-GO 4** 로 갈렸고, 원본의
`GO / findings 0` 은 **2번만** 재현됐다. `a-code` 가 매 런 다른 실제 결함을 지목한다는 기존
관측(「불안정이 아니라 탐색 폭」)의 뒷면이다 — **탐색 폭이 넓다는 것은 한 번의 GO 가 커버리지를
보장하지 않는다는 뜻이다.**

새 배선은 없다. `run_round.py --merge-into <라운드> --reviewers a-code` 가 이미 그 동작을 하고,
**GO 라운드에서만 병합이 곧 합집합**이다 — `--merge-into` 는 그 축의 이전 findings 를 덮으므로
(`run_round.py:1696`), 정의상 `regression` 0 인 GO 후보에서만 `0 ∪ 새 샘플 = 새 샘플` 이 성립한다.
규칙·건너뛰는 자리·중복 처리는 `references/resample.md`.

### ✨ red-team·pr-triage — NO-GO 교착을 `ABORTED` 로 닫고 미해결 P1 을 트리아지로 잇는다 (#65)

라운드를 17회·13회 돌고도 NO-GO 에서 멈춘 채 PR 이 나간 사례가 5건이고 거기서 탈출 결함 29건이
나왔다. 남은 P1 이 기획·PO 판단을 기다려 **코드로는 닫을 수 없는 교착**이었는데, `ABORTED` 마커는
전체 기록을 통틀어 1건만 쓰였다. 그러면 마지막 상태가 NO-GO 로 남아 **「수렴 실패」와 「정당한
중단」이 구분되지 않는다.**

`assets/aborted-template.md` 를 만들고 `## 미해결 P1` 절을 넣었다(*비워두지 않는다. 정말 없으면
`없음` 이라고 적는다 — 빈 절은 「확인했고 없다」와 「확인하지 않았다」가 구분되지 않는다*).
`pr-triage` 는 그 목록과 겹치는 지적을 「이미 알고 나간 것」으로 가른다 — **사실 축은 그대로
리뷰어가 맞고 분류만 `out-of-scope`** 다. 기계가 읽는 것은 여전히 마커 파일뿐이다.

### ✨ pr-triage — 게이트 기록이 없는 브랜치에 안내 한 줄 (#64)

탈출 결함의 10%(18건)가 **게이트를 아예 안 돌린 PR** 에서 나왔다. red-team 은 선택이므로
차단하지 않고 사실만 보인다. 기록이 있으면 무음이고, ABORTED 마커나 인계 코멘트가 있으면
안내를 내지 않는다 — 판정 자체가 실패했을 때도 무음이다(*확인하지 못한 사실을 안내로 단정하지
않는다*).

### 🚫 red-team — 6번째 축 `b6-async-race` 는 만들지 않는다 (#52)

비동기 경합 계열 13건이 나왔고 12건이 `none` 이라 축 신설을 검토했으나 **기각**했다.
등급·중복을 정리하면 유효·고유 10건이고, 위 15건을 머지한 뒤 대조하면 미덮임 7건인데
**그중 6건이 `ceo-client#874` 한 PR 클러스터**다. 그리고 재현 실험에서 `b1` 이 6/6 침묵해
「표 채우는 축을 늘리면 잡는다」가 실측으로 성립하지 않았다. 대신 `b1` 의 조건부 열에 네 번째
트리거로 재귀속했다. 재검토 조건은 `references/design.md` 에 적어 뒀다 — **`#874` 밖의 서로
다른 PR 3건 이상에서 나오면 다시 센다.**

### 📊 red-team — 보강 후 실측: GT 회수 0, 대신 새 결함 5 (#82)

위 15건은 **「탈출한 결함이 다음에는 잡힌다」** 를 전제로 넣었다. 같은 replay 배선으로 재봤고
그 전제는 지지되지 않았다.

| 샘플 | verdict | `a-code` | `b2` | GT 적중 |
|---|---|---|---|---|
| 보강 전 | NO-GO | P2 ×1 | — | 0/6 |
| 보강 후 ×3 | NO-GO ×3 | P2·P2·(P1+P2) | P2 ×1 (1/3) | **0/6** |

**GT 회수는 3샘플 합집합에서도 0** 이다. `b2` 발화는 1/3 이라 **효과로 볼 수 없다.**

**원인은 「탐색하지 않음」이 아니다** — raw 리뷰어 출력을 열어 보니 리뷰어는 GT 파일을 전부
읽고도 findings 를 내지 않았다(arm D3 `b2`: `AuthenticationScreen.tsx` 38회 ·
`useSendbirdAuthentication.ts` 26회 · `GroupChannelMessageEmpty.tsx` 16회). 무엇이 억제하는지는
미규명이다.

다만 **발화한 5건이 전부 #55·#58 계열**이고, 그중 하나(`Navigation.tsx` 가 거짓 전제 위에 섰다)는
2샘플에서 재현되며 P1 으로 올라온 **코드로 확인된 실제 결함**이다. 즉 보강은 「놓친 것을 회수」가
아니라 **「같은 계열의 다른 것을 새로 찾는」** 방향으로 작동했다. 규칙 추가가 무용하다는 뜻은
아니지만, 이 작업의 성과를 전자로 서술하면 틀린다.

함께 `references/evidence.md` 에 **「replay 배선」** 절을 만들었다 — 워크트리 · **그 브랜치
lockfile 로 `yarn install --immutable`**(빠뜨리면 오늘의 의존성에는 참이고 리뷰 대상 시점에는
거짓인 finding 이 나와 실험이 무효가 된다) · 재현 검증 · 합성 base 커밋 · GT 에서 봉인 항목 거르기 ·
판정 기준 사전 기입.

## 2026-09-07

### 🐛 red-team — 봉인이 판정 요청을 덮어 회귀 2건이 GO 를 통과했다 (#48)

자진 신고 절(`## 이번 라운드에서 특히 볼 것` = 검증 대상)과 `## 스코프 밖`(지적 금지)이 **같은
주제를 다룰 때 어느 쪽이 이기는지** 규약이 없었다. 실측(`zigbang/ceo-client` B2C-54308 /
PR #958)에서는 봉인이 이겼고, 자진 신고로 올린 질문이 리뷰어 5명 전원에게 봉인돼
**code-8·code-10 이 findings 0 으로 GO 한 뒤 PR 리뷰가 production 회귀 3건**을 잡았다.
#38 의 교정은 `## 계획 게이트 생략 근거` 한 자리에만 걸려 있어, 상시 절인 `## 스코프 밖`
쪽으로 같은 실패 모드가 재발했다.

- **`prompts/_common.md` — 판정 요청이 봉인을 이긴다.** "특히 볼 것"·"자진 신고" 로 지목된
  주제가 스코프 밖 문구와 겹치면 판정한다. 봉인은 *무엇을 할지가 확정됐다*는 뜻이지
  *그렇게 짠 코드가 맞다*는 뜻이 아니다 — 확정된 정책을 뒤집는 지적만 하지 않으면 된다.
- **`assets/context-template.md` — `## 스코프 밖` 아래에 「여기 적으면 안 되는 것」.**
  외부에서 확정된 기획·정책은 `### 기획·정책 확인 완료 사항` 으로 보내고, 이 절에는 **이
  티켓에서 손대지 않는 파일·기능만** 적는다. 실측 컨텍스트는 그 슬롯을 비워 둔 채 정책을
  봉인 절에 적어 구현 검증까지 함께 닫았다.
- **`run_round.py` — `## 스코프 밖` 밖에서 봉인을 선언한 절 제목을 경고**(`seal_headings`).
  같은 봉인을 별도 최상위 절로 한 번 더 쓰는 것은 그 항목을 강하게 닫으려는 신호이고, 실측에서
  정확히 그 항목이 회귀였다. 라운드는 막지 않는다(⚠ 만 찍는다). `--from-zax` 초안의
  `## 스코프 밖` 안내도 같은 문장으로 맞췄다.
- **식별자 교집합 검사는 기각했다** — 이슈 제안 3번은 `srsltid` 가 양쪽 절에 등장한다고
  봤지만 원문 `code-9/context.md` 의 `## 스코프 밖` 에는 없다(그 봉인은 "발동 집합의 내용
  자체" 라는 주제 단위다). 실측에서 발화하지 않는 검사는 넣지 않았다. 근거는
  `references/evidence.md`.
- `test_context_check.py` 에 회귀 검사 추가(중복 봉인 절 경고, `## 이미 반영된 지적
  (재제기 금지)` 오탐 없음).


### 🐛 pr-triage — 인계 코멘트가 게시만 되고 소비되지 않았다 (#46)

`external-sync.md` 의 작업 C 는 게이트 기록을 PR 코멘트로 내보내 「의사결정 기록이 없는 PR」
**열화 경로를 정상 경로로 되돌린다**고 선언했는데, 그 복귀가 구현돼 있지 않았다.
`fetch_comments.py` 의 `record` 수집은 `~/.red-team/runs2/`(= `resume.latest_round`) 만 봤고
`red-team handoff: <repo>/<branch>` 앵커를 읽는 코드는 저장소 전체에 0건이었다. 결과적으로
인계 코멘트를 남겨도 다른 기기·새 워크트리·다른 사람의 세션은 여전히 `의사결정 기록: 없음`
으로 떨어져 코드로만 판정했다 — 작업 C 를 돌린 값이 사라진다.

- 로컬 라운드가 없을 때 **이미 받아온 코멘트 페이로드**에서 앵커를 찾아 `record` 를 채운다
  (`{"source": "handoff-comment", "comment_id": …, "anchor": …, "body": …}`). 추가 API 호출은
  없다. 로컬 기록에도 `"source": "local"` 을 붙여 원천이 출력·JSON 양쪽에서 갈린다.
- **폴백이지 대체가 아니다** — 로컬 라운드가 하나라도 있으면 폴백은 발동하지 않는다. 인계
  코멘트는 로컬 기록의 렌더라 항상 같거나 더 낡다. 그래서 요약 출력에 "더 낡을 수 있다" 를
  함께 찍는다.
- 앵커 판정은 `fp_markers` 와 같은 엄격함을 쓴다 — 코드펜스·blockquote 안의 앵커는 인용이라
  세지 않는다. 인계 본문은 프롬프트를 접힌 블록의 펜스에 두고 앵커는 그 밖 첫 줄이라
  (작업 C 절차 1) 펜스를 걷어내도 앵커는 남는다.
- 앵커 2건 이상이면 **keep-first + 중복 보고** — 작업 B·C 와 같은 규약이다.
- 스캔은 `--new-only` **필터 전** 전체 items 에서 한다. 인계는 내가 올린 코멘트라 필터 뒤에는
  남지 않는다(fp_seq 가 같은 이유로 필터 전에 세는 것과 같은 함정).
- `--show-scope` 가 인계 코멘트 본문에서도 `## 스코프 밖`·`## 후속 티켓` 을 뽑는다. 그래서
  렌더에서 **그 절 이름을 바꾸면 안 된다** — 바꾸면 조용히 빈손이 된다. `external-sync.md` 에
  계약으로 못 박았다.
- 앵커 부재·파싱 실패는 예전과 똑같이 `의사결정 기록: 없음` 으로 떨어진다 — 「red-team 기록이
  없어도 동작한다」 계약은 그대로다.
- 문서는 두 스킬을 함께 갱신했다(한쪽만 고치면 갈라진다): `external-sync.md` 작업 C 에 소비
  경로와 앵커·절 이름 계약, `pr-triage/SKILL.md` 3절·「의사결정 기록이 없는 PR」 절,
  `pr-triage/references/edge-cases.md` 에 「인계 코멘트 폴백」 상세.
- `test_fetch_comments.py` 에 회귀 검사 추가(로컬 우선·폴백 채택·앵커 부재·keep-first
  중복 보고·`--new-only` 뒤에서도 잡힘·`--show-scope` 절 추출·인용 앵커 제외).

## 2026-09-04

### 🐛 red-team — `## 이번 라운드에서 특히 볼 것` 이 TODO 표시 없이 이월된다 (#42)

`resume.py --next` 가 손 갱신 대상으로 표시하던 절은 `## 리뷰 대상`·`## 검증 상태` **둘뿐**
이었고, 안내 문구도 「2개만 남았다」라고 못 박아 나머지를 갱신하지 않는 것이 정상 절차처럼
읽혔다. 그래서 `## 이번 라운드에서 특히 볼 것` 은 직전 라운드 판 그대로 넘어갔다.

인벤토리 표(#33)와 성격이 다르다. 인벤토리는 **서술**이라 코드와 어긋나면 코드가 이기지만
(`long-gate.md` §6), 이 절은 **살아있는 지시**다 — 낡으면 리뷰어가 잘못된 것을 검사해 그
라운드의 5축 실행이 통째로 빗나간다. 실측(ceo-client PH-1270, code-7): 그 라운드가 방금
삭제한 **라인번호 12개를 설치본과 대조하라**는 지시가 이월돼 있었고, 앞선 라운드에서는 아무도
눈치채지 못했다.

- `PER_ROUND` 에 `## 이번 라운드에서 특히 볼 것` 추가. 기존 메커니즘을 그대로 탄다 —
  TODO 마커가 붙고, 중복 절 정리(마지막만 남김)도 같이 적용된다. 템플릿에서 **선택** 절이라
  없는 컨텍스트에서는 조용히 통과한다.
- `carry_forward` 의 하드코딩 튜플을 `PER_ROUND` 로 통일. 상수와 분기가 갈라져 있던 것이
  애초에 이 누락이 생긴 자리다.
- 완료 안내 문구를 **`PER_ROUND` 에서 파생**하도록 바꿨다 — 절 개수·이름을 실제 컨텍스트에서
  세므로 상수만 늘리면 문구가 따라온다. 「2개만」처럼 숫자를 손으로 적어 두면 다음 절 추가가
  또 같은 방식으로 어긋난다.
- `test_carry_forward.py` 에 회귀 검사 추가(TODO 부착·선택 절 부재 시 무변·멱등·중복 정리).

## 2026-09-02

### ✨ red-team·pr-triage — 게이트 기록을 PR 로 내보낸다: **인계 코멘트**(작업 C) + 판정 기준 절 신설 (#40)

`SKILL.md` 는 「첫 컨텍스트를 쓰는 것이 세션 간 인계 그 자체」라고 선언하는데, 그 `context.md`
는 `~/.red-team/runs2/` **로컬에만** 있다. 다른 기기·새 워크트리·다른 사람의 세션은 읽지
못한다. 그래서 pr-triage 에는 「의사결정 기록이 없는 PR」 절이 따로 있었다 — 기록을 못 읽으면
3·4절을 건너뛰고 코드로만 판정하는 **열화 경로**다. 관찰된 사용 패턴(사내 PR 에서 마무리 전
「새 세션 인계 프롬프트」를 접힌 블록으로 남기는 것)은 그 기록을 PR 로 내보내 **열화 경로를
정상 경로로 되돌린다.** 새로 쓰는 문서가 아니라 이미 있는 기록의 렌더다.

- `references/external-sync.md` 에 **작업 C — 인계 코멘트** 신설. 새 스크립트는 없다 —
  같은 파일의 작업 A(티켓 본문)·작업 B(AC 코멘트)·pr-triage 9절(PR 본문)이 이미 쓰던
  **승인 규약·앵커 멱등성·keep-first·write 실패 처리를 그대로 상속**한다(「셋이 한 벌」이
  「넷이 한 벌」이 됐다). 담을 것 6종을 원천 절과 함께 표로 고정하고, 브랜치의 **전 라운드**를
  순회한다(마지막 GO 라운드만 보면 근거가 앞 라운드에 남는다).
- **트리거는 게이트 GO 직후가 아니라 pr-triage 9절과 같은 사람 신호**(approve·LGTM 또는
  사용자 지시)다. GO 직후에 두면 트리아지 라운드가 계속 돌며 코드가 바뀌어 인계가 즉시
  낡는다 — approve 가 코드가 멈추는 첫 지점이다. 이후 커밋이 붙으면 앵커로 **재생성**한다
  (#33 의 「라운드마다 인벤토리 배너 재생성」과 같은 처방). 앵커
  `red-team handoff: <repo>/<branch>` 에 **라운드 번호를 넣지 않는다** — 브랜치 생애 전체에
  하나이고, 번호를 넣으면 라운드마다 새 코멘트가 쌓인다.
- **게시 위치는 PR 코멘트이고 티켓이 아니다.** 이어받는 대상이 브랜치이므로, 같은 티켓을
  여러 브랜치가 공유하면 앵커가 겹쳐 남의 인계를 덮는다(작업 B 에 이미 기록된 함정). 같은
  이유로 작업 C 는 `## 티켓` 이 비어 있어도 돌고, 파일 상단의 게이트 노트를 **작업 A·B
  한정**으로 분리했다. `red-team` 을 안 거친 PR(`context.md` 부재)은 조용히 건너뛴다.
- **작업 B 와 겹치지 않게 독자로 갈랐다** — B 는 「무슨 AC 를 무엇으로 충족했나」(리뷰어·QA),
  C 는 「재조사 없이 이어받을 것」(다음 세션). 후속 티켓은 원천이 같으므로 C 에서 재서술하지
  않고 B 코멘트를 링크한다. 그리고 **가장 값이 큰 것은 「다시 하지 말 것」**(기각한 대안)이라고
  문서에 박았다 — 반영 목록만 나열한 인계는 커밋 로그와 중복이라 값이 없다.
- **부수로 dangling 절 참조 6건을 해소했다.** pr-triage 3절 표, 작업 A·B,
  `references/plan-drift.md`, `scripts/run_round.py` — **넷이** `## 판정 기준`·`### 불변식`·
  `### 기각한 대안`·`### 기획·정책 확인 완료 사항`·`## 철회한 주장` 을 근거로 부르는데
  `assets/` 템플릿에는 **하나도 없었다.** 작업 C 의 렌더 원천이 정확히 그 절들이라 안 고치면
  「반영/후속 티켓」만 나열하는 껍데기가 된다. `context-template.md` 에 `## 판정 기준
  (Spec AC · Gherkin)` 절과 하위 3절, `decisions-template.md` 에 `## 철회한 주장` 을 신설하고
  pr-triage 3절 표의 locator 표기 2건(`기획·…` 생략형, `###` 레벨 오기)을 정정했다.
- **`tools/check_skill_docs.py` 에 검사 5 신설** — 백틱으로 인용한 절 이름이 `assets/`
  템플릿·SKILL.md 에 실제로 있는지 **스킬 경계를 넘어** 대조한다(pr-triage 3절 표가
  red-team 템플릿의 절을 부른다). 기존 검사 2가 파일 참조를 보는 것의 절 단위 대응이다.
  이 검사가 돌자마자 위 6건 외에 **`## 계획 전문`·`## 판정 기준 — 문서 6-구조 + 계획
  생존성 커버리지`** 두 건을 더 잡아 같이 신설했다(`references/zax.md`·
  `references/plan-coverage.md` 가 부르던 조건부 절). 해소 판정은 앞부분 인용을 허용하되
  (`## 판정 기준` ← `## 판정 기준 (Spec AC · Gherkin)`) **낱말 경계에서만** 인정한다 —
  경계를 안 걸면 `### 불변식` → `### 불변식들` 개명이 무음으로 통과한다(음성 테스트로 실측).
  우리 템플릿의 슬롯이 아닌 절 3건(스크립트 생성 `## Diff 스냅샷`, PR 본문의
  `## 리뷰 반영`, 파싱 오인 예시로 든 `## 중단 근거`)은 `SECTION_ALLOWLIST` 에 **이유와
  함께** 등재했다. `--selftest` 로 판정 로직만 따로 돌 수 있고, `AGENTS.md` 에 편집 규칙을
  짝으로 남겼다(검사기만 있으면 존재를 모르고, 규칙만 있으면 안 따른다 — #6 의 결정).
- **pr-triage `SKILL.md` 본문은 건드리지 않았다.** 토큰 예산이 4,981/5,000 이라 새 줄을 넣을
  여유가 없었고, 9절이 이미 `references/edge-cases.md`(9절 상세)를 가리키므로 **기존 포인터에
  얹었다.** red-team 쪽은 참고 문서 표의 external-sync 행 하나만 고쳐 오히려
  4,977 → **4,970 토큰**으로 줄었다.

검증: `tools/check_skill_docs.py` OK(pr-triage 4,983 / red-team 4,970, 한도 5,000). 참조되는
절 이름 14개를 템플릿과 전수 대조해 미해소 0건, stale locator 0건. 스크립트 변경이 없어
`scripts/test_*.py` 는 돌리지 않았다. **이 변경 자체에는 게이트를 돌리지 않았다** — #38 의
safe-harbor 기준으로는 계획 게이트 대상이다(문서 간 모순·프로토콜 논리구멍이 결함 모드다).

...
- **3단 폴백**: ① json 태그 펜스(기존 동작, 1순위 유지) → ② 언어 태그 없는 펜스 →
  ③ bare JSON — 정방향 span-skip `raw_decode`(성공한 객체의 끝으로 점프해 중첩 내부
  객체가 판정으로 오인되지 않고, 정규식 균형 카운팅의 문자열-리터럴 문제도 없다).
  각 경로에서 마지막 수락 후보가 이긴다 — 서술 중간 예시 JSON 보다 진짜 판정이 이긴다.
  bare 는 **문서를 끝내는 객체만** 수락한다 — 산문 속 형식 예시를 미완주 리뷰어의
  판정으로 오인하지 않기 위한 경계이고, 실측 사고의 판정은 서술 마지막에 있었다.
  파싱에 실패한 span 은 통째로 건너뛴다(문자열·escape·여닫이 종류(stack) 인식 반복문
  스캐너) — 깊은 중첩에서 여는 중괄호마다 재파싱하던 비선형 폭증과, 깨진 외곽 객체의
  내부 조각(짝 안 맞는 closer 포함)을 판정으로 오인하는 경로를 함께 없앤다.
- **수락 조건 강화**: dict + `findings` 가 dict 만 담은 list + `verdict` 가 str(세
  ...
  `count_access_errors`(pre-existing 이지만 extract_json 보다 먼저 불려 생존 보장을
  우회하던 자리) 공통.
...
- 테스트: `test_extract_json.py` 신규(33건 — 기존 경로 회귀 방지, 실측 bare 형태,
  마지막 판정 우선, 수락 조건, 예시 오인 거부, 중첩·거대 정수·극단 중첩·문자열 안
  중괄호, span-skip 성능·깨진 외곽 조각 거부).

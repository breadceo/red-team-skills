...
- **3단 폴백**: ① json 태그 펜스(기존 동작, 1순위 유지) → ② 언어 태그 없는 펜스 →
  ...
  파싱에 실패한 span 은 통째로 건너뛴다(문자열·escape 인식 반복문 스캐너) — 깊은
  중첩에서 여는 중괄호마다 재파싱하던 비선형 폭증과, 깨진 외곽 객체의 내부 조각을
  판정으로 오인하는 경로를 함께 없앤다.
...
- 테스트: `test_extract_json.py` 신규(32건 — 기존 경로 회귀 방지, 실측 bare 형태,

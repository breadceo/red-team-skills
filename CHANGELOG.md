...
- **3단 폴백**: ① json 태그 펜스(1순위 유지) → ② 언어 태그 없는 펜스 → ③ bare JSON.
  ...
  파싱에 실패한 span·`[` 배열 컨테이너는 통째로 건너뛴다(여닫이 종류(stack) 인식
  반복문 스캐너 + **인용부호 의심 정책** — 실패 span 은 유효 JSON 이 아니라 문자열
  문법을 알 수 없으므로, 인용부호가 있으면 closer 를 신뢰하지 않고 문서 끝까지
  건너뛴다). 깊은 중첩의 비선형 폭증, 깨진 외곽 객체·미닫힌 배열의 내부 조각(짝 안
  맞는 closer·인용부호 안 closer 포함) 오인을 함께 없앤다. 전 경로가 입력 길이에
  선형이다(opener 위치 캐시 포함).
...
- 테스트: `test_extract_json.py` 신규(45건 — 기존 경로 회귀 방지, 실측 bare 형태,
  마지막 판정 우선, 수락 조건, 예시 오인 거부, 중첩·거대 정수·극단 중첩·문자열 안
  중괄호, 깨진 외곽·미닫힌 배열 조각 거부, 선형 시간 바운드 4종, parse_output·
  count_access_errors 생존).

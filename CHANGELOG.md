...
### 📝 red-team — 계획 게이트의 **면제를 없앤다**: 생략 조건 safe-harbor + plan/code 빈도 표 (#38)

`SKILL.md` 는 「계획 게이트를 건너뛰지 않는다」를 규칙으로 선언했지만 그 게이트의 트리거는
「구현 전, **계획 문서가 있을 때**」였다. **계획 문서를 안 쓰는 선택이 곧 게이트 면제**여서
규칙이 자기충족적으로 무력화됐다. 실측(2026-09-01T08:28Z, `report_usage.py`): 685 라운드 중
plan 비율 41%, plan 0% 인 namespace 가 셋, 16% 가 하나 — **그중 하나가 이 스킬 자신의
저장소다.** 문서만 봐서는 규칙을 어긴 것인지 쓸 일이 아니었던 것인지 구분되지 않았다.

- SKILL.md 「두 개의 게이트」에 **「계획 문서를 쓰나 — 계획 게이트 판별」** 절 신설.
  구조는 **「쓴다」 열거가 아니라 safe-harbor** 다 — **기본은 쓴다**, 생략은 AND 조건 2개를
  모두 만족할 때만(① 결론이 교차가 아니라 **후보 중 선택** ② 실패 모드를 잡는 **기계 검사의
  이름을 댈 수 있다**). 「쓴다」를 열거하는 구조는 보완집합이 무한해 완전할 수 없다 — 이
  변경 자체의 계획 게이트에서 행을 2→3개로 늘려도 리뷰어가 매 라운드 새 빈칸을 찾아냈다.
  기존 예외(`references/zax.md`: PRD·Architecture·Spec 에는 걸지 않는다)는 그대로 두고
  맞물리게 했고, `/task plan` 산출물처럼 **계획 문서가 이미 있으면 판별 없이 탄다**.
  `fix-flow` 가 부른 게이트는 생략하지 않는다(저장소 밖 스킬이라 계약을 맞출 수 없다).
- `assets/context-template.md` 에 **`## 계획 게이트 생략 근거`** 절 신설. `## 스코프 밖` 에
  적으면 안 되는 이유: 그 절은 리뷰어에게 *지적 금지* 로 전달되므로 「이 테스트가 실패
  모드를 잡는다」를 잘못 적으면 그 틀린 커버리지 주장이 **반박 불가능**해진다.
- `scripts/report_usage.py` 에 **게이트 종류별 절** 추가 — namespace 별 plan/code 라운드
  수와 plan 비율. 그룹 키는 **(루트, 최상위 namespace)** 이고 **자동 병합하지 않는다**:
  namespace 문자열만 쓰면 `runs/x` 와 `runs2/x` 가 합산되고, 짧은 이름을 substring 필터로
  반복 호출해 합계를 내면 `ceo-client` 가 `zigbang__ceo-client` 까지 잡아 **661 라운드/$248
  이 969/$362 로 부풀었다.** suffix 일치는 소유 증거가 아니므로(`team-a/app` 대
  `team-b/app`) `(구)` 마커 + 각주로 사람에게 넘긴다. 라운드 종류·라운드 수는 **JSON 파싱
  전에** 경로만으로 세고(손상 `round.json` 이 조용히 0% 를 만들지 않게), 조기 종료는
  `rounds == 0` 일 때만이며 헤더 괄호에 `배정 집계`·`파싱 실패`·`구버전`·`plan/code 아닌
  라운드` 를 나눠 적는다 — 세 표의 분모가 다른 것이 의도된 설계임을 드러낸다.
  **이 표는 변경 단위 생략률이 아니다**(라운드 수 ≠ 변경 수) — 주장은 「plan 라운드가 0인
  namespace 가 눈에 띈다」 하나다.
- 근거·실측·기각한 대안은 `references/evidence.md` 의 「계획 게이트를 건너뛰지 않는 이유」
  절로. SKILL.md 토큰 예산(≤5,000)은 「루프」절의 **상한 5 초과 네 패턴**을
  `references/long-gate.md` §0 으로 옮기고(§1~§6 진입 조건과 구분되게 「상한 5 초과에만
  적용」을 명시), `## 게이트 GO 후` 절이 `references/external-sync.md` 와 **중복**이던 것을
  걷어내 맞췄다 — 규칙 문장은 하나도 지우지 않았다.

검증: 이 변경 자체를 **계획 게이트 7라운드**(`a-plan` + `b5-plan-ordering`, codex)로 돌려
결함 20건을 반영하고 plan-7 GO. `--gate plan` 을 고른 이유는 문서 변경의 결함 모드가 코드
버그가 아니라 문서 간 모순·프로토콜 논리구멍이라서다(코드 게이트 b1~b4 는 빈손으로 온다).

---

## 2026-08-26

### 📝 red-team — 장기 게이트는 **건수가 아니라 지적의 성격**으로 읽는다 (#32)

한 티켓에서 코드 게이트를 **19라운드** 돌린 실측이 나왔다. production 동작 결함은
`code-13` 이 마지막이었고, 이후 6라운드는 P1 0 인 채로 지적의 성격만
`테스트 커버리지 → 주석·문서` 로 옮겨갔다. 그런데 **P2 건수는 1 → 10 으로 늘었다** —
악화처럼 보이지만 10건이 5개 결함이고 production 은 0건이었다. 추이표가 건수만 담으면
"수렴 중인가 악화인가" 를 가를 근거가 없다.

더 비쌌던 것은 **개별 패치가 뿌리를 못 건드린 것**이다. 고친 자리가 다음 라운드에 다시
지적되고 건수가 5 → 7 → 10 으로 늘었다. 뿌리는 코드가 아니라 **같은 불변식이 여러 파일에
중복 서술**된 것이었고, 정본 1곳 + 나머지 포인터로 한 번 정리한 것이 22건의 뿌리였다.
기존 「라운드 상한 5」의 세 에스컬레이션 패턴(불변식 / pre-existing 층 / same-origin)에는
이 계열이 없었다.

- SKILL.md 「루프」절에 **성격 전이 축**(`production → 테스트 커버리지 → 주석·문서`,
  역행이 경고)과 **네 번째 에스컬레이션 패턴**(같은 계열 3라운드 비감소 → 개별 패치 중단,
  사람이 정본 1곳 + 포인터로 한 번 정리) 추가.
- 새 문서 `red-team/references/long-gate.md` — **라운드가 3회를 넘으면** 읽는다. 성격 기록,
  다음 라운드 판정 기준 선기입, 자진 신고 항목의 명시 이관, N라운드 무응답 항목의 사람 판정
  출구 다섯 규약.
- `assets/decisions-template.md` 에 `## 검증 구멍`·`## 수렴 추이`(성격 열)·
  `## 다음 라운드의 판정 기준` 세 절 추가 — 자진 신고는 **면제가 아니라 예약**이라 그
  라운드에 닫거나 명시 이관하고, 탈출 조건은 결과를 보기 **전에** 적는다.
- `assets/context-template.md` 에 `## 이번 라운드에서 특히 볼 것` 절 추가 — 3라운드 연속
  어느 축도 다루지 않으면 무응답은 "못 본다" 가 아니라 **"판정할 것이 없다"** 는 신호다.
  사람이 판정하고 `한 문장 + 근거` 를 코드 주석과 `재제기 금지` 양쪽에 싣는다.
- 실측 전문: `red-team/references/evidence.md`. SKILL.md 예산(5,000 tok)은 references 와
  중복된 근거 서술을 걷어내 맞췄다 — 규칙은 하나도 지우지 않았다.
...
"인용 없는 주장은 무효" 규칙이 **있는 상태로** 뚫렸다. B2C-53629 계획서는 인용을 달았고
(`@sendbird/uikit-chat-hooks` — 파일도 줄 내용도 정확했다), 틀린 것은 **그 파일이 실행되지
않는다**는 것이었다. 실제 호출부는 같은 이름의 훅을 export 하는 `@sendbird/uikit-tools` 를
import 한다. **"최근접 설치 사본" 규칙도 이 케이스를 가르지 못한다** — 둘 다 최근접이고
둘 다 실재하기 때문이다.

계획 게이트 **12라운드**가 이 결함을 놓쳤고, 코드 게이트 첫 라운드에서 P1 이 나왔다
(채널 전환 후 A 채널의 실패 메시지가 B 대화방으로 발송된다 — `a-code`·`b2-interaction`
독립 수렴). 메커니즘은 **인용이 검증의 종결자로 작동한다**는 것이다: `파일:줄` 이 붙어
있으면 리뷰어가 거기서 멈춘다. 12라운드 동안 아무도 "그 파일이 실행되긴 하나"를 묻지 않았다.

그래서 계약을 "인용을 요구한다" 에서 **"인용의 도달을 요구한다"** 로 올렸다. 도달 경로를
함께 적게 하면 그 경로 자체가 검증 대상이 되어, 인용이 종결자에서 주장으로 내려온다.
확인 비용은 grep 한 줄이다.

- `assets/context-template.md` — 도달성 조항 신설 + 외부 신호 인용에 `도달:` 필드 추가.
- `SKILL.md` — 인벤토리 규칙을 "인용 없는 주장은 무효이고, **도달을 보이지 않은 인용도 무효**" 로.
- `prompts/a-plan.md`·`a-code.md`·`b5-plan-ordering.md` — 도달성 검사 지시. **리뷰어 프롬프트는
  `SKILL.md` 를 읽지 않으므로** 계약을 선언한 곳과 별개로 전부 갱신했다(계약 소비처 전수 전파).
- `references/evidence.md` — 사례와, 기존 규칙이 구조적으로 못 잡은 이유.
...
- **3단 폴백**: ① json 태그 펜스(1순위 유지) → ② 언어 태그 없는 펜스 → ③ bare JSON.
  ...
  패스라 lazy 정규식의 O(n²) 재탐색이 없다. bare 는 통합 정책이다 — **모든 opener(인라인 포함)가
  유효 파스 연쇄에 포함**되고(인라인을 무시하면 인라인에서 열린 미종결 컨테이너
  안의 조각이 오인된다), **후보 자격은 줄 시작(선행 공백 허용) opener 뿐**이라
  산문(미종결 인용 등)에 파묻힌 인라인 조각은 판정이 될 수 없다. 성공한 객체의
  끝으로 점프해 중첩 내부 객체가 판정으로 오인되지 않고,
  각 경로에서 마지막 수락 후보가 이긴다(서술 중간 예시 JSON 보다 진짜 판정).

...
혼자 쓴 코드는 이미 옳다고 믿는 축에서만 검토된다 — 그래서 축을 나눠 **독립된 리뷰어를
동시에** 돌리고, **리뷰어는 지목한 곳만 본다**(근거: `references/evidence.md`).

## 두 개의 게이트

| 게이트 | 언제 | 리뷰어 |
|---|---|---|
| **계획** | 구현 전 — 계획 문서를 쓰는 변경일 때(아래 판별) | `a-plan` + `b5-plan-ordering` |
| **코드** | 구현 후, 커밋·PR 전 | `a-code` + `b1-state-matrix` + `b2-interaction` + `b3-visibility` + `b4-null-propagation` |

### 계획 문서를 쓰나 — 계획 게이트 판별

**계획 게이트를 건너뛰지 않는다.** 트리거가 「계획 문서가 있을 때」이므로 **계획 문서를 안
쓰는 선택이 곧 게이트 면제다** — **기본은 쓴다.** 생략은 아래 두 조건을 **모두** 만족할
때만이다(이유·실측: `references/evidence.md`).

| 생략 조건 (둘 다) | 확인 방법 |
|---|---|
| 결론이 여러 관점의 **교차**가 아니라 **후보 중 선택**이다 | 축을 나눠 봐야 나오는 판단(생존성·상태 매트릭스·신뢰 경계·순서·마이그레이션)이 없다 |
| 그 변경의 **실패 모드를 기계 검사가 이미 잡는다** | `tools/check_skill_docs.py`·CI·린터·타입·테스트 중 **무엇이 잡는지 이름을 댈 수 있다** |

- **하나라도 못 대면 쓴다.** "작아서"·"문서라서"·"국소라서"는 조건이 아니다.
- **이미 계획 문서가 있으면 판별 없이 탄다** — zax `/task plan` 산출물이 전형이다.
- **`fix-flow` 가 부른 게이트는 생략하지 않는다.**
- 생략했으면 컨텍스트의 **`## 계획 게이트 생략 근거`** 절에 두 조건을 각각 한 줄로 적는다.
  **`## 스코프 밖` 이 아니다** — 그 절은 *지적 금지* 지시라 틀린 근거가 반박 불가능해진다.
- PRD·Architecture·Spec 은 이 표 밖이다 — `a-plan` 은 *구현 계획* 리뷰어다(`references/zax.md`).
...
- `## 변경 대상 인벤토리` — **라운드 수를 줄이는 장치.** 목록을 뽑은 **명령을 함께
  적는다.** 외부 신호·SDK 주장은 템플릿의 근거 규칙을 따른다 — **인용 없는 주장도, 도달을
  보이지 않은 인용도 무효다.**(이유: `references/evidence.md`)
...
**포인터 파일은 없다** — `<repo키>/<branch키>` 가 곧 키다(설계 이유:
`references/evidence.md`). "이미 반영된 지적"은 **직전 라운드의 `round.json` 을 그대로
읽는다** — 기억에 의존하지 않는다. **`--out` 은 eval 용 별도 경로다.**

### 부분 실패가 나면 — 라운드를 버리지 않는다

`PARSE-FAIL`·파일접근오류가 한두 축에만 나면 **`references/recovery.md` 를 읽고 그 축만
`--merge-into` 로 재실행해 원 라운드에 병합한다.** `round.json` 을 손으로 고치지 않는다.
...
**`references/fix-writing.md` 를 읽고 그 세 검사를 통과시킨다. 심각도와 무관하다**
(P2·P1 양쪽에서 나왔다).
...
`보류` 가 비어야 다음 라운드로 간다 — 미결을 두고 돌리면 리뷰어가 그것을 다시 보고한다.
...
라운드 상한은 5다. 넘어가면 프롬프트를 더 돌리는 게 답이 아니다 — 먼저 건수가 아니라
**지적의 성격**을 본다. `production → 테스트 커버리지 → 주석·문서` 로 옮겨가면 수렴,
**역행이 경고다.** **3라운드를 넘었으면 `references/long-gate.md` 를 읽고 따른다** — 성격
기록, 판정 기준 선기입, 자진 신고 이관, 무응답 항목의 출구, 그리고 **상한 5를 넘겼을
때만 적용하는 네 패턴과 각각의 교정**.

## 다른 세션에서 이어받기

구현 세션과 리뷰 세션의 **분리가 권장 기본**이다 — 라운드의 입력은 `(저장소 경로,
context.md)` 뿐이고 둘 다 디스크에 있다(비용 실측: `references/evidence.md`). 이어받을 때는
이것만 실행한다:
...
게이트 전환은 `python3 "<red-team-skill>/scripts/resume.py" --next code` — 컨텍스트를
**누적 이관**한다(덮으면 이전 지적이 되살아난다). 남는 것은 `<!-- TODO(resume) -->`
표시된 `## 리뷰 대상`·`## 검증 상태` 두 절뿐 — 채운 뒤 스크립트가 출력한 `run_round.py` 명령을 `--out` 까지 그대로 실행한다.

**자동 전달되지 않는 유일한 것은 변경의 의도다.** **첫 컨텍스트를 쓰는 것이 세션 간 인계
그 자체다** — 리뷰어는 무엇이 *의도된 동작*인지 알 수 없다.

...
기록, 판정 기준 선기입, 자진 신고 이관, 무응답 항목의 출구.
...
- **같은 계열이 3라운드 비감소** → **개별 패치를 멈춘다.** 뿌리가 "같은 사실이 여러 곳에
  흩어져 있다" 면(불변식은 이미 있고 서술만 중복) 라운드는 표면만 깎는다. **사람이 한 번에**
  정본 파일을 정하고 나머지를 포인터로 바꾼 뒤 다음을 확인 라운드로 쓴다.
  판별: **직전 라운드에 고친 자리가 다시 지적되는가.**

## 게이트 GO 후 — 외부 산출물 최신화

`## 티켓` 이 채워진 라운드가 GO 를 받으면 **`references/external-sync.md` 를 읽고 그대로
수행한다** — 트리거 조건과 작업이 거기 있다. 승인 없이 write 하지 않는다. `## 티켓` 이
없거나 비면 조용히 건너뛴다(정상 케이스다).
...
| `references/long-gate.md` | **라운드가 3회를 넘었을 때** — 성격 전이·탈출 조건 규약 |

...
## a-code 실패 경로에 실패 격리·자원 수명을 넣은 이유 — issue #60

같은 분류(`~/.red-team/eval/escape/all-classified.json`, 186 finding, 2026-09-09)에서 이 처방으로
묶인 행은 4건이다. 근거 PR: ceo-client#927, zigbang-client#9596.

**유효 근거 4건** — 등급이 갈린다. ceo-client#927 2건은 **post-round 탈출(GO 전)**, zigbang-client
#9596 2건은 **post-GO 탈출**이다. 판정도 갈린다 — #927 은 `partial:a-code`, #9596 은 `none`.

| 축 | PR | 결함 | 판정 / 등급 |
|---|---|---|---|
| 실패 격리 | ceo-client#927 | `failedResendQueue.ts:308` 의 `entry.resend()` 가 항목별 try 밖에 있어 동기 throw 하면 그 flush pass 전체가 중단되고 뒤 항목이 그 재연결에서 재전송되지 않는다 | `partial:a-code` / post-round |
| 〃 | ceo-client#927 | 같은 자리를 다른 리뷰어가 재제기 — 바깥 `.catch(() => undefined)` 가 예외만 삼켜 같은 pass 의 나머지 항목이 pending 으로 남고, 추가 재연결이 없으면 자동 재전송 기회를 잃는다 | `partial:a-code` / post-round |
| 자원 수명 | zigbang-client#9596 | 2초 폴백 타이머는 `Promise.race` 만 끝낼 뿐 `createLocationMeta` 가 시작한 통합검색 요청을 취소하지 않아, 응답 반환 후에도 SDK 재시도가 최대 45초 동안 내부 검색 API 연결을 점유한다 | `none` / post-GO |
| 〃 | zigbang-client#9596 | hermes 가 3회차로 재제기한 동일 결함 — 앞선 out-of-scope 회신이 오분류였다 | `none` / post-GO |

`partial` 과 `none` 의 차이가 두 축을 한 문단에 넣은 근거다. #927 2건이 인용한 기존 문장은
「에러·실패 경로를 특히 본다 … 크래시가 없는 것과 기능이 조용히 막히는 것은 다르다」 — 실패
경로를 보라고는 하지만 **무엇을 나열하라는 지시가 없어** partial 로 남았다. #9596 2건은
`evidence` 가 비어 있다(a-code 에 대응 문장이 없다). 그래서 새 절을 세우지 않고 그 문단 **뒤에
붙였다** — 「실패 경로에 둘을 더 대입한다」로 시작해 partial 로 판정된 문단이 열거 대상을 갖게
한다.

### 자리를 「어떻게 찾나」의 마지막으로 고르고 다섯 번째 방향 문단으로 만들지 않은 이유

후보는 셋이었다 — 1번(새 회귀)·2번(논리구멍) 같은 번호 항목, 「어떻게 찾나」의 다섯 번째
방향 문단, 그리고 그 절의 마지막 문단.

**번호 항목을 기각한 이유는 1~5번이 결함의 *종류*라는 것이다** — 회귀·논리구멍·사실오류·관측·
선언된 보장. 이 두 축은 종류가 아니라 **열거 절차**(「전수 나열한다」)이고, 그것을 담는 절이
「어떻게 찾나」다. 2번에 넣으면 「이 변경이 막으려는 결함을 막지 못하는 경로」라는 정의가
루프 순회 절차까지 뜻하게 된다.

**다섯 번째 방향 문단을 기각한 이유는 은유가 닫혀 있다는 것이다.** 네 문단은 호출부(누가 쓰나)
→ 형제(옆·뒤) → 소비처(앞) → 매처(입구)로 **공간 방향**이 완결돼 있고, 각 문단이 그 방향을
문장에 명시한다(「남은 방향은 **앞**이다」·「남은 것은 **입구**다」). 실패 격리는 방향이 아니라
**범위**(한 항목 → 나머지)이고 자원 수명은 **시간**이다. 다섯 번째로 세우면 그 문단만 방향을
선언하지 못해 은유가 무음으로 깨진다 — 그래서 방향 네 문단이 끝난 뒤, 이미 실패 경로를 다루는
마지막 문단에 이어 붙였다.

### 두 축을 한 문단에 묶고 4건을 열거하지 않은 이유

분류기가 준 처방 문장은 3개이고 그중 둘은 같은 말(「취소되지 않고 남는 요청·타이머·구독을 전수
나열」)이다. 4건을 4줄로 펼치면 실패 격리 2줄이 같은 자리(`entry.resend()`)의 중복이 되고 자원
수명 2줄이 같은 문장이 된다. 실제로 다른 것은 **범위**와 **수명** 둘뿐이라 굵은 표제 둘로 묶었다.
`a-code.md` 는 이미 길고, 근거가 4건인 이번 처방이 6건짜리(#57)·10건짜리(#59) 처방보다 길어질
근거가 없다.

### b2-interaction·issue #54 와 무엇이 다른가

세 자리와 붙어 있어 경계를 적어 둔다. 이 PR 은 `b2-interaction.md` 를 건드리지 않았다.

| 문장 | 대상 | 도착점 |
|---|---|---|
| b2 — 「잠금 플래그(`pending`·`ref`)를 세운 뒤 예외가 나는 경로에서도 그것이 풀리는지」 (#61) | **한 컨트롤**의 잠금 | 그 컨트롤이 죽는다 — 사용자가 다시 누를 수 없다 |
| **a-code 범위 — issue #60** | **루프·배치**가 순회하는 항목 목록 | 뒤 항목들이 소비되지 않는다 — 누른 적도 없는 항목이 조용히 남는다 |
| b2 — 「이탈한 뒤 도착하는 응답」 (#61) | 늦게 온 응답이 **실행하는 것**(전역 다이얼로그·`router.push`) | 사용자가 엉뚱한 화면에서 무언가를 본다 |
| **a-code 수명 — issue #60** | 이탈 뒤에도 **살아 있는 자원**(요청·타이머·구독·워커) | 비용·경합·중복 발사 — 사용자에게 보이지 않는다 |
| a-code 소비처 계약 — issue #54 | 그 값의 **크기**(상한 없음으로 적고 내부 API·비용 자원으로 전달되는 경로) | 큰 값 하나가 하류를 깬다 |
| **a-code 수명 — issue #60** | 그 요청의 **수명**(몇 초 점유하는가) | 끝난 줄 알았던 요청이 계속 돈다 |

#927 의 `entry.resend()` 건은 b2 의 잠금 문장으로는 도달하지 않는다 — 그 자리에서 잠금은
정상적으로 풀린다(`startResend` 의 `catch` 가 `releaseResend` 를 부른 **뒤** 의도적으로
re-throw 한다). 깨지는 것은 잠금이 아니라 **flush pass 의 나머지**이고, 그것은 컨트롤 목록에
없다. 반대로 #9596 은 컨트롤이 없는 자리다 — SSR `getInitialProps` 에는 누를 것이 없다.
...
| 결함 | 결함 커밋 → 정정 커밋 | 대조 결과 |
|---|---|---|
| #927 항목 단위 격리 부재 | `4c072bb0e` → `60b7f02d9` | 결함 커밋 `apps/CeoApp/src/lib/failedResendQueue.ts:308` 이 `const started = entry.resend()` 로 맨몸이고, 항목마다 도는 `try` 는 `:321` 에서 시작해 `await Promise.race([Promise.resolve(started).catch(() => undefined), …])` **만** 감싼다 — 비동기 reject 는 감싸고 동기 호출은 그 밖이다. 그 예외가 `for` 두 겹을 지나 `:342` 의 `.catch(() => undefined)` 까지 올라가므로 같은 스냅샷의 뒤 항목이 소비되지 않는다. 던지는 쪽은 의도적이다 — `components/chat/message/FailedResendRegistrar.tsx:229-233` 의 `catch (error) { failedResendQueue.releaseResend(...); throw error }` 에 「onPress 가 동기 throw 하면 잠금이 남아 사용자가 다시 탭할 수 없게 된다」가 붙어 있다. 결정적으로 같은 커밋 `:265-267` 의 flush JSDoc 이 「재전송 결과는 관측하지 않는다 … try/catch 는 그 계약이 바뀌었을 때 **pass 전체가 끊기지 않게 하는 방어다**」라고 서술한다 — 방어를 선언한 주석과 try 의 실제 범위가 어긋난다. 정정 커밋이 `entry.resend()` 를 try 안으로 넣으며 「그 호출이 bounded wait 의 try 밖에 있어 예외가 tail 의 catch 까지 올라갔다. 같은 pass 의 뒤 항목이 전부 소비되지 않는다. rejected Promise 와 같게 다룬다」를 남겼다. 「항목마다 도는 try 의 범위가 던지는 호출을 실제로 감싸는지 대조한다」가 지목하는 대조가 `:308` ↔ `:321` 이다 |
| #9596 취소되지 않는 요청 | `27a44df11` → `a264d6ba2` | 결함 커밋 `packages/zigbang-www/pages/stay/map.tsx:74-88` 이 `Promise.race([lookup, new Promise((resolve) => { timer = setTimeout(… resolve({}) , META_TIMEOUT_MS) })])` 이고 `finally { clearTimeout(timer) }` 로 **타이머는 걷는다** — 걷지 못하는 것은 `lookup` 이다. 피호출자 `packages/screens/src/lib/DynamicMetaTag/index.tsx:165` 의 `searchAPI.searchTotal({ q, …searchType })` 은 인자에 timeout·signal 자리가 없어 취소 수단이 호출 경로에 없다. 남는 쪽의 점유 시간은 같은 파일 `:21-22` 의 JSDoc 이 적어 둔다 — 「SDK 는 5s 부터 1s 씩 늘려 재시도해 최대 45s 를 기다린다(`@zigbang/api` HttpClientProvider `DEFAULT_TIMEOUT`/`MAX_TIMEOUT`)」. **표면은 이미 diff 안에 문장으로 있었다** — 같은 커밋 `:34-35` 의 `MAX_KEYWORD_LENGTH` JSDoc 이 「조회는 상한을 넘겨도 **취소되지 않아** 내부 검색 API 로 부하가 그대로 전달된다」라고 쓰고, 그 처방으로 검색어 길이 상한만 걸었다. 정정 커밋이 stay 한정 무재시도 provider 를 주입하며 「호출부는 Promise.race 로 2s 에 폴백하지만 race 는 진 쪽을 취소하지 않는다 / searchTotal 은 FetchOptions 를 받지 않아 timeout 을 넘길 수 없다 → 검색 장애 시 이미 응답을 끝낸 요청들이 소켓과 내부 검색 API 를 계속 점유한다」를 남겼다. 「취소 수단이 호출 경로에 있는지(피호출자가 옵션 인자·signal 을 받는지) 시그니처를 열어 확인하고, 남는 쪽이 … 몇 초 점유하는지 그 클라이언트의 재시도·타임아웃 설정에서 읽는다」가 지목하는 두 확인이 `index.tsx:165` 의 시그니처와 `map.tsx:21-22` 의 45s 다 |

두 번째 행이 이 처방의 필요성을 가장 직접적으로 보인다 — 취소되지 않는다는 사실이 diff 안
JSDoc 에 이미 적혀 있었는데도 게이트가 GO 를 냈다. 3번(사실오류)은 서술과 코드의 어긋남을
보고, 이 문장이 없으면 **서술과 코드가 일치하는 자리**(정확히 취소되지 않는다고 적혀 있다)는
아무 축에도 걸리지 않는다.

**`red-team/SKILL.md` 는 건드리지 않았다.** 바뀐 것은 `a-code.md` 와 이 문서뿐이라 예산
검사 대상이 아니다.

## b4 의 열거 대상을 기존 nullable·쓰기 순서·truthy 빈 값으로 넓힌 이유 — issue #62

같은 분류(`~/.red-team/eval/escape/all-classified.json`, 186 finding, 2026-09-09)에서 이 처방으로
묶인 행은 3건이고 **전부 post-GO 탈출**이다. 판정은 셋 다 `partial:b4-null-propagation` —
b4 가 발화는 했는데 그 값을 열거 목록에 넣지 못했다는 뜻이다. 근거 PR: ceo-client#872,
account-sdk-reactnative#46, zigbang-client#9596.

| 방향 | PR | 결함 | 인용된 기존 문장 |
|---|---|---|---|
| 열거 대상 — 기존 nullable | ceo-client#872 | `useAdUserData.ts:23` 의 `violationSummary?.lastPenaltyInfo.status === IN_EXPLANATION_PERIOD` 가 병렬 쿼리 로딩 구간에서 `undefined → false` 로 떨어져, users/me 가 먼저 준비되면 페널티 사용자가 등록 게이트를 통과한다 | 「`x > 0`, `x === n` — 없음이 조용히 **거짓**이 된다」 |
| 쓰기 순서 | account-sdk-reactnative#46 | `Store.ts` 가 `인증정보_삭제하기()` 로 메모리의 `zauth` 를 먼저 비운 뒤 `logout(this._authData.zauth \|\| "")` 를 불러 서버 revoke 에 빈 문자열이 나가고 서버 세션이 남는다 | 「`catch` 가 값 대신 없음을 돌려주게 된 곳 … 각 값에 대해 **소비처까지 끝까지** 따라가서」 |
| 반대 방향 — truthy 빈 값 | zigbang-client#9596 | 공백만 있는 검색어(`"   "`)가 truthy 라 `if (!keyword)` 가드를 통과해 의미상 빈 질의가 검색 API 로 나가고 400 + Sentry 이벤트를 만든다 | 「`!x` / `!x && y` — `!undefined === true` 다. 없음이 **참**으로 읽힌다」 |

세 자리를 각각 다르게 놓았다. 성격이 다르기 때문이다.

- **기존 nullable → 열거 대상 선언(맨 위)에 문장 하나.** 이 축은 첫 문장이 곧 목록의 정의라
  거기서 빠진 값은 뒤의 4패턴·핵심 질문을 아무리 잘 대입해도 도달하지 않는다. 실제로 #872 는
  `x === n` 패턴을 **정확히** 인용받고도 partial 로 남았다 — 대입할 패턴은 맞았고 대입할 값이
  목록에 없었다.
- **반대 방향 → 첫 번째 패턴 항목에 이어 붙였다.** #9596 이 인용한 문장이 바로 그 항목이고,
  둘은 같은 가드(`!x`)의 양쪽 오독이다 — `undefined` 를 참으로 읽는 쪽과 `"   "`·`[]`·`{}` 를
  거짓으로 읽지 않는 쪽. 5번째 항목으로 세우면 같은 가드가 목록에 두 번 오른다.
- **쓰기 순서 → 패턴 목록 뒤의 독립 두 줄.** 나머지 둘은 「그 값이 무엇에 들어가는가」인데
  이것은 「그 값이 언제 읽히는가」다. 4패턴은 전부 대입 자리를 가리키는 목록이라 순서 결함을
  항목으로 끼우면 목록의 축이 섞인다. 소비처 추적이 끝난 자리에 이어 붙였다.

### 분류기 처방의 「새로 참여시킨」을 「손댄 판정식에 참여하는」으로 바꾼 이유

분류기가 준 문장은 「이 변경이 판정식에 **새로 참여시킨** 기존 nullable 값」이다. 원본 커밋을
열어 보면 이 건은 그 형태가 아니다 — PR base(`512131ead`)의 `useAdUserData.ts:19` 에도 같은
`violationSummary?…` 줄이 이미 있었다. 이 변경이 한 것은 **판정식을 다시 쓴 것**이다:
옆 값(`data` → `judgment`)을 3-상태(`loading`/`error`/`ready`)로 갈라 「판정 불가」를 분리하면서,
같은 판정식에 남아 있던 기존 nullable 하나는 그대로 두었다. 「새로 참여시킨」으로 적으면 이
건이 그 지시에 걸리지 않는다. 「손댄 판정식·분기에 참여하는」이 두 형태를 다 덮는다.

### 왜 3건을 3줄로 나열하지 않았나

근거가 3건이니 문장도 3개면 될 것 같지만, 셋 중 둘은 **이미 있는 문장의 확장**이다 — 열거
대상 선언과 첫 패턴 항목. 새로 세운 것은 쓰기 순서 두 줄뿐이고, 프롬프트 순증은 6줄이다
(4건에 9줄을 쓴 #60 보다 짧다). 3줄 나열은 「기존 문장에 얹으면 되는 것」까지 새 항목으로
올려 목록의 축을 흐린다 — #57 이 6건 중 1건을 새로 쓰지 않고 기존 문장에 얹은 것과 같은 처리다.

### `a-code`·`b1`·`b2` 와 무엇이 다른가

네 자리와 붙어 있어 경계를 적어 둔다. 이 PR 은 `a-code.md`·`b1-state-matrix.md`·
`b2-interaction.md` 를 건드리지 않았다.

| 문장 | 대상 | 도착점 |
|---|---|---|
| a-code 소비처 계약 (#54) | 그 값의 **길이·형식 상한** | 큰 값·형식 밖 값이 하류를 깬다 |
| **b4 열거 대상 (#62)** | 그 값의 **유무·의미**(없음인가, 의미상 없음인가) | 없음이 판정 결과로 읽힌다 |
| a-code 자원 수명 (#60) | 자원이 **살아 있는가**(요청·타이머·구독) | 끝난 줄 알았던 것이 계속 돈다 |
| **b4 쓰기 순서 (#62)** | 같은 값의 **읽기·쓰기 순서** | 지운 뒤 읽어 기본값이 하류로 나간다 |
| b1 매트릭스 (#59) | 사용자가 그 칸에서 **무엇을 읽는가** | 화면에 틀린 문구·수량이 뜬다 |
| **b4 (#62)** | 그 값이 **판정식에서 무엇으로 읽히는가** | 판정 자체가 뒤집힌다 |
| b2 잠금 플래그 (#61) | **컨트롤**이 다시 눌리는가 | 사용자가 다시 누를 수 없다 |

#872 건이 b1 과 가장 가깝다 — 같은 PR 의 b1 소관 결함(「로그인한 사용자가 조회 끝나기 전
미인증 분기로 떨어진다」)과 같은 로딩 구간을 본다. 갈라지는 곳은 **판정식이냐 칸이냐**다.
b4 가 보는 것은 `validateRegister()` 가 `true` 를 돌려준다는 사실 자체이고(등록이 진행된다),
b1 이 보는 것은 그 결과로 어떤 다이얼로그가 뜨는지다. #872 의 이 건은 다이얼로그가 **안 뜨는**
쪽이라 b1 의 칸에는 채울 것이 없다 — 게이트가 그냥 통과한다.

### 손 대조 3건 (원본 커밋)

새 지시가 실제 결함의 표면을 열거하는지 결함이 존재했던 커밋에서 확인했다.

| 결함 | 결함 커밋 → 정정 커밋 | 대조 결과 |
|---|---|---|
| #872 로딩 중 `undefined` 가 판정 통과 | `146cdcaea` → 같은 PR `466272c2f` 가 **옆 값만** 갈랐다 / `violationSummary` 는 정정 없음(main 에도 그대로) | 결함 커밋 `apps/CeoWeb/hooks/house/useAdUserData.ts:14` 가 `const { data: violationSummary } = useGetMyViolationSummary({})` 로 로딩 플래그를 받지 않고, `:23` 의 첫 분기가 `violationSummary?.lastPenaltyInfo.status === …IN_EXPLANATION_PERIOD` 다. 이 커밋이 새로 붙인 `:20-22` 주석이 순서 불변식을 선언하며 「penalty 는 users/me 가 아니라 getMyViolationSummary(**독립 출처**)에서 오므로 users/me 가 실패해도 **사실**이다」라고 쓴다 — 그 독립 출처에도 자기 로딩 구간이 있다는 것은 적혀 있지 않다. 결정적으로 같은 PR 의 뒤 커밋 `466272c2f` 가 **옆 값**(`judgment`)에 대해서만 「판정 불가」를 갈라 낸다 — `judgment.state === "loading"` → `fetch_loading`, `"error"` → `fetch_error`, 주석 「아직 실패가 아니다 — 실패로 단정하면 users/me timeout 재시도 구간 … 내내 '불러오지 못했습니다' 가 나간다」. 같은 판정식 안에서 한 값은 3-상태로 갈리고 다른 한 값은 `undefined → false` 로 남았다. 두 쿼리는 병렬이라 users/me 가 먼저 `ready` 이고 `violationSummary` 가 아직 in-flight 인 조합이 도달 가능하다. PR base `512131ead:19` 에 같은 줄이 이미 있어 「새로 생기거나 nullable 성이 바뀐 값」으로는 열거되지 않는다 — 「손댄 판정식·분기에 참여하는 기존 nullable 값도 나열한다」가 지목하는 값이 이것이다 |
| #46 삭제가 읽기보다 먼저 | `9c0ef6234` → **정정 없음(main `:667` 에 그대로)** | 결함 커밋 `packages/login-store/Store.ts:183-191` 의 `인증정보_삭제하기()` 가 `this.setAuthData({ zauth: null, access_token: null })` 로 메모리를 **동기로** 비운다. 그 호출이 `:540` 이고, 4줄 뒤 `:544` 가 `await LoginModel.instance.logout(this._authData.zauth \|\| "")` 다 — 읽는 시점에 `zauth` 는 이미 `null` 이라 `\|\| ""` 가 빈 문자열을 revoke 로 보낸다. 같은 파일의 **형제 로그아웃 경로**는 반대로 되어 있다: `:601` 이 `const 요청_zauth = this._authData.zauth ?? ""` 로 삭제 전에 스냅샷을 잡고, 최종 커밋 `4c4e52553:781-783` 이 그 자리에 「인자는 현재값이 아니라 **캡처값**이고(⑥)」를 명시한 채 `logout(요청_zauth)` 를 부른다. 한 파일 안에서 한 경로는 캡처하고 다른 경로는 삭제 뒤에 읽는다. 「이 변경이 추가·이동한 상태 삭제·초기화가 같은 값의 이후 읽기보다 먼저 실행되는지, 그 읽기가 `?? ""`·`\|\| ""` 로 기본값이 되어 하류로 나가는지」가 지목하는 대조가 `:540` ↔ `:544` 다 |
| #9596 truthy 빈 값이 falsy 가드 통과 | `c0979f7af` → `27a44df11` | 결함 커밋 `packages/zigbang-www/pages/stay/map.tsx:43-46` 이 `const keyword = Array.isArray(raw) ? raw[0] : raw` 다음 `if (!keyword) { return {} }` 로, 정규화 없이 falsy 검사만 한다. 바로 위 `:40-42` 주석은 `string[]` 로 들어오는 경우를 이미 다뤄 뒀다(「그대로 넘기면 통합검색이 500 을 준다」) — **입력 형태를 의심한 흔적이 있는데 공백만 있는 값은 빠졌다**. 정정 커밋이 `:58` 을 `(Array.isArray(raw) ? raw[0] : raw)?.trim()` 으로 바꾸며 「공백만 있는 검색어(`?search_keyword=%20%20%20`)는 truthy 라 `!keyword` 를 통과한다. 그대로 넘기면 통합검색이 400 `Invalid Q` 를 주고 apiInterceptors 가 Sentry 이벤트를 남긴다」를 남겼다. 「공백 문자열·빈 배열·빈 객체는 의미상 없음인데 truthy 라 이 가드에 걸리지 않는다」가 지목하는 자리가 `:44` 의 `!keyword` 다 |

세 행 중 앞의 두 행이 **main 에 아직 남아 있다**. post-GO 탈출 3건이 전부 partial 이었던 것과
같은 이유로 읽는다 — 축은 맞게 발화했고, 그 값이 열거 목록에 없었다.

**`red-team/SKILL.md` 는 건드리지 않았다.** 바뀐 것은 `b4-null-propagation.md` 와 이 문서뿐이라
예산 검사 대상이 아니다.

## b3 를 색 외 상속 속성과 override 되돌림까지 넓힌 이유 — issue #63

`~/.red-team/eval/escape/all-classified.json` (2026-09-09, 186 finding 분류) 의 `b3-visibility`
행에서 나온 처방이다. 이슈 표는 3건이지만 **유효 근거는 2건**이다 — 리뷰어 코멘트
`created_at` 과 라운드 완료 시각을 대조해 다시 가른 결과 zuix2#956 `키보드 표시 중 불필요한
safe-area 여백 잔존` 은 게이트가 돌기 전에 달린 코멘트(pre-gate)로 빠졌다. 12개 이슈 중 근거가
가장 얇다. 남은 2건은 전부 post-GO 탈출이고 둘 다 같은 PR(ceo-client#916)이다.

| PR | 결함 | 탈출 판정 | 등급 |
|---|---|---|---|
| ceo-client#916 | 하위 직접 선언이 컨테이너 상속을 이겨 대상 화면이 미수정으로 남음 | `partial:b3-visibility` | post-GO |
| ceo-client#916 | 호출자 override 가 클래스 병합 때문에 반쪽만 먹힘 | `partial:b3-visibility` | post-GO |
| zuix2#956 | 키보드 표시 중 불필요한 safe-area 여백 잔존 | `none` | **pre-gate — 근거 아님** |

### 이 축은 정상 동작 중이라 물량을 늘리지 않았다

`b3-visibility` 는 전체 498 findings 중 **12건만** 냈지만 **12/12 가 P1, 11/12 가 regression**
이다. 다른 축과 비교하면 발화량이 한 자릿수인데 P2 가 0건이다. 이것은 축이 약하다는 신호가
아니라 설계대로 도는 신호다 — 이 프롬프트의 마지막 줄(「이 변경에 새 UI 요소가 전혀 없으면
findings 는 `[]` 다. 억지로 만들지 않는다」)이 UI 없는 변경에서 빈 손으로 끝내게 만들고, 그래서
남는 것이 전부 실제 대비·가려짐 결함이다.

**그래서 이 PR 의 목표는 축을 더 시끄럽게 만드는 것이 아니다.** 열거 대상을 넓혀 P2 물량을
만들면 12/12 P1 이라는 이 축의 성질이 먼저 깨진다 — P2 가 섞이기 시작하면 사람이 이 축의
finding 을 「일단 일괄 승인」 쪽으로 읽게 되고, 그 순간 regression 11건을 P1 으로 올려 사람 앞에
세우던 효과가 희석된다. 놓친 표면 2건만 정확히 추가하고, 프롬프트 순증을 **2줄**로 닫았다
(3건에 6줄을 쓴 #62 보다 짧다 — 근거가 그보다 얇으니 문장도 그보다 적어야 한다).
마지막 줄은 한 글자도 건드리지 않았다.

### 자리 — 1건은 새 줄, 1건은 기존 문장에 얹었다

**하위 직접 선언 → 5단계 3번에 한 줄.** 3번은 「호출부마다 **조상 방향으로 올라가며** 실제로
색을 칠하는 첫 조상을 찾는다」다. 이 결함은 같은 상속 탐색인데 **방향과 속성이 둘 다 반대**다 —
조상이 아니라 하위를 보고, 색이 아니라 `word-break` 를 본다. 3번이 이미 「상속 사슬을 따라간다」
자리이므로 새 항목을 세우지 않고 그 안에 한 줄로 붙였다. 6번째 단계로 세우면 같은 상속 탐색이
순서 목록에 두 번 오른다.

**override 되돌림 → 5단계 1번의 `tailwind-merge` 문장에 얹었다.** 처방 후보를 그대로 새 항목으로
세우기 전에 겹침을 먼저 확인했고, **겹친다**. 1번에는 이미 「`tailwind-merge` 같은 병합이 끼면
**어느 쪽이 이기는지**까지 확인한다」가 있다. 이 결함이 묻는 것도 병합 해소 결과이고, 갈라지는
곳은 **방향 하나**다 — 기존 문장은 「내가 쓴 두 클래스 중 어느 쪽이 남는가」를 묻고, 이 결함은
「호출부가 표준 유틸로 되돌리려 할 때 무엇이 밀려나고 무엇이 남는가」를 묻는다. 같은 `cn()` 한
번의 결과를 양쪽에서 읽는 것이라 문장을 새로 세우면 tailwind-merge 가 프롬프트에 두 번 등장한다.
AGENTS.md 의 중복 금지에 따라 기존 문장을 이어 「호출부가 표준 유틸로 되돌릴 때 내가 쓴 표기가
되돌리려는 유틸과 같은 병합 그룹인지까지」로 늘렸다.

### pre-gate 1건(zuix2#956 키보드·safe-area)을 넣지 않은 이유

세 가지가 같은 방향을 가리켜서 뺐다.

1. **근거 등급이 pre-gate 다.** 게이트가 GO 를 내기 전에 달린 코멘트라 「축이 놓쳤다」의 증거가
   아니다. 이 계열을 근거로 인정하면 게이트가 아직 보지도 않은 결함으로 프롬프트를 늘리게 된다.
2. **판정 방식이 다르다.** 지금 b3 가 내는 숫자는 대비비 하나이고, 근거는 색 두 개다. 위치·여백은
   키보드 높이·safe-area inset·컨테이너 기하를 실기기 조건별로 계산해야 나오는데, 리뷰어가 코드만
   읽고 그 값을 낼 수 없다. 열거는 되고 판정은 안 되는 지시가 되어 「의심스러움」류 P2 를 만든다.
3. **위 §이 축은 정상 동작 중… 과 정면으로 충돌한다.** 열거 대상이 늘어나는 폭이 이번 2건과
   비교가 안 된다 — 새 UI 요소가 없어도 「키보드가 뜨는 화면인가」는 거의 항상 참이라
   마지막 줄의 빈 손 종료가 무력화된다.

**어느 축도 이것을 보지 않는다는 사실은 남겨 둔다.** `red-team/prompts/*.md` 8개 전수에
`키보드`·`safe-area`·`여백` 이 0건이다. 이 계열이 post-GO 탈출로 다시 잡히면 그때는 b3 가
아니라 새 자리를 논해야 한다 — 대비 축에 기하 판정을 얹는 것이 아니라.

### 손 대조 2건 (원본 커밋)

새 지시가 실제 결함의 표면을 열거하는지 **결함이 존재했던 커밋**에서 확인했다. ceo-client#916 은
커밋이 둘이고, 뒤 커밋 `d51c4a86f` 가 리뷰 반영본이라 대조는 앞 커밋 `7c0d52e76` 에서 했다.

| 결함 | 결함 커밋 → 정정 커밋 | 대조 결과 |
|---|---|---|
| 하위 직접 선언이 컨테이너 상속을 이김 | `7c0d52e76` → 같은 PR `d51c4a86f` | 결함 커밋 `apps/CeoWeb/components/ui/alert-dialog.tsx:57` 이 `AlertDialogContentWithMobile` 에 `break-keep [overflow-wrap:anywhere]` 를 단다. 공지 모달의 컨테이너가 바로 그것이고(`components/sidebars/modal/NoticeList.tsx:73`), 제목은 `:75` `AlertDialogTitle` 이라 상속을 받는다. 본문은 `:93` `<div className={styles.item_wrapper}>` 이고, 같은 커밋 `notice.module.css:6,17,27` 이 `.item_wrapper :global(.item_text_content1\|benefit\|content2)` 세 곳에 `word-break: break-all` 을 **직접 선언**한다 — 한 모달에서 제목만 고쳐진 상태다. 정정 커밋이 그 세 줄을 삭제했다. 「색 외 상속 속성을 컨테이너에서 바꿨으면 하위의 직접 선언도 전수 찾는다」가 지목하는 대조가 `alert-dialog.tsx:57` ↔ `notice.module.css:6,17,27` 이다 |
| 호출자 override 가 반쪽만 먹힘 | `7c0d52e76` → 같은 PR `d51c4a86f` | 결함 커밋의 테스트 `apps/CeoWeb/__tests__/components/ui/dialog-word-break.test.tsx:134` 는 `break-all` **한 방향만** 단언한다 — 「호출자가 break-all 을 주면 break-keep 만 밀려나고 overflow-wrap 은 남는다」에 `expect(classes).toContain("[overflow-wrap:anywhere]")` 와 주석 「예외를 준 자리에서도 무공백 토큰 overflow 는 계속 막아야 한다」. 즉 남는 것을 **의도**로 적었다. 빠진 것은 **되돌리는** 방향이다: 표준 유틸 `break-normal` 은 tailwind-merge 의 `break-normal\|words\|all\|keep` 그룹에서 `break-keep` 만 밀어내고 임의 속성 `[overflow-wrap:anywhere]` 는 다른 그룹이라 남는다. 정정 커밋이 `:152` 「break-normal 로는 overflow-wrap 을 되돌릴 수 없다」·`:162` 「`[overflow-wrap:normal]` 로는 되돌릴 수 있다」 2건과 각 Content 위 계약 주석을 추가했다. 같은 커밋의 파일 상단 주석(`:28`)은 병합 그룹을 **내가 쓴 클래스가 사라지는 쪽**으로만 설명한다 — 「호출부가 표준 유틸로 되돌릴 때 내가 쓴 표기가 되돌리려는 유틸과 같은 병합 그룹인지까지」가 지목하는 자리가 그 반대 방향이다 |

두 행 모두 **같은 PR 안에서 리뷰어가 잡아 고쳤다.** 게이트가 GO 를 낸 뒤 사람 리뷰가 잡았다는
것이 `partial` 판정의 내용이다 — 축은 이 PR 에 맞게 발화했고(모달 텍스트 표시 변경이므로),
상속 사슬을 색 한 속성으로만 따라갔다.

**`red-team/SKILL.md` 는 건드리지 않았다.** 바뀐 것은 `b3-visibility.md`(+2줄)와 이 문서뿐이라
예산 검사 대상이 아니다.

## 비동기 경합을 b1 조건부 열과 a-code·b2 로 재귀속한 이유 — issue #52

같은 분류(`~/.red-team/eval/escape/all-classified.json`, 186 finding, 2026-09-09)에서 이 계열로
묶인 행은 13건이고 12건이 `covered_by: none` 이었다. **신규 축 `b6-async-race` 를 만드는 안을
검토했고 기각했다** — 기각 근거 두 개와 재검토 조건은 `references/design.md` 의
「6번째 축 `b6-async-race` 도 만들지 않는다」 절에 있다. 이 절은 그 결정 **이후**의 실측, 즉
어느 GT 가 어느 축의 어느 자리로 갔는지와 그 자리를 고른 이유를 남긴다.

### 등급·중복 정리 — 13건 → 유효·고유 10건 → 미덮임 7건

| 처리 | 건수 |
|---|---|
| **pre-gate** — ceo-client#904 확정 직전 재검증 없는 stale 스냅샷 확정(TOCTOU). 코멘트 시점에 완료된 라운드가 없어 「축이 놓쳤다」의 근거가 아니고, 동일 결함이 2회 등재돼 있었다 | −2 |
| **중복 등재** — ceo-client#927 등록-소비 순서(`5028267358`·`5422585864`, 같은 결함 2회) | −1 |
| **유효·고유 근거** | **10** |
| 그중 #53~#66 머지 후 프롬프트에 **덮임** — ceo-client#874 「폐기된 실행이 `setIsLoading(false)` 를 실행해 미인증 보호 콘텐츠 노출」은 b1 의 「로딩이 끝났다고 판단하는 조건과 실제로 데이터가 도착하는 조건이 다르다」에 걸린다(분류의 `covered_by` 도 `b1-state-matrix`) | −1 |
| ceo-client#884 「언마운트/블러 커밋 전 평가된 포커스 가드가 stale 안내를 전역 UI 로」 — #61 이 b2 에 넣은 「이탈한 뒤 도착하는 응답 … cleanup 에서 세우는 플래그는 라우터 전환이 커밋되기 전 창을 닫지 못한다」가 덮는다 | −1 |
| **이 이슈가 처방한 것** — 미덮임 7건 + 부분 2건 | **8** |

**유효 근거 8건** — post-GO 탈출 7, post-round 1(GT 10). pre-gate 2건은 처방 후보로만 읽고
근거 건수에 넣지 않았다(#59 가 ceo-client#957 을, #61 이 ceo-client#984 를 처리한 것과 같다).
그 2건이 요구하는 「확정 직전 재검증」은 조건부 열의 「부수효과를 적용하기 직전에 그 입력이 아직
최신인지 확인하는 코드가 있는지」에 우연히 걸리지만, 그 문장을 만든 근거는 GT 6 이다.

| GT | 간 자리 | PR | 결함 | 판정 |
|---|---|---|---|---|
| 1 | b1 열 — 폐기된 실행이 남기는 것 | ceo-client#874 | 외부 `finally` 가 stale 로 폐기된 실행에도 `settledPathRef.current = pathAtStart` 를 무조건 기록해, A→B→A 내비게이션에서 A 가 「판정 완료」로 표시된 채 재인증이 영구 차단된다 | `none` |
| 6 | 〃 | ceo-client#874 | `runAuthenticate` 가 시작 시점 경로를 클로저로 캡처해 `redirectTo` 를 정하고, 완료 시 시작 경로 확인 없이 `router.replace` 와 30일 쿠키 쓰기를 실행한다 | `none` |
| 3 | b1 열 — 최신 입력의 재판정이 도는지 | ceo-client#874 | 실행 중 경로가 바뀌면 새 경로 effect 는 in-flight 가드로 즉시 반환하고, 기존 실행의 `finally` 는 ref 만 바꿔 리렌더를 보장하지 않아 최신 경로가 미판정으로 남는다 | `none` |
| 4 | 〃 | ceo-client#874 | 완료 후 `setAuthError(null)`·`setIsLoading(false)` 가 **동일 값** 갱신이라 React 가 렌더를 생략해, 재판정을 상태 변화에 의존하는 effect 가 다시 돌지 않는다 | `none` |
| 7 | b1 열 — 가드와 주체의 수 | ceo-client#874 | stale 판정 가드의 기준 `latestPathRef` 가 render 가 아니라 passive effect 에서 갱신돼, commit 직후 effect 실행 전에 이전 Promise 가 resolve 하면 가드가 무력이다 | `none` |
| 5 | 〃 | ceo-client#874 | `onSessionError` 가 복구 connect 를 await 없이 호출해 중첩 실행되고, 완료 순서에 따라 늦게 끝난 이전 실행이 `connectionLost` 를 덮는다 | `none` |
| 9 | 〃 — 승자·패자 | ceo-client#884 | 세션 복구 connect 와 화면 cold connect 가 경쟁해 진 쪽만 `setConnectPhase("failed")` 를 찍고 이긴 쪽은 성공 콜백이 `undefined` 라, 채팅은 연결됐는데 「채팅에 연결할 수 없습니다」가 남는다 | `partial:b1-state-matrix` |
| 2 | **a-code** 「입구」 문단 | ceo-client#874 | `authenticate` 가 매 렌더 새 `queryKey`/`queryFn` 에 의존해 참조가 바뀌고 `isRunningRef` 만 해제되므로, 완료된 인증이 리렌더에서 다시 실행되고 캐시되지 않는 account 실패는 재요청된다 | `none` |
| 10 | **b2** 자동 경로 문단 | ceo-client#927 | 연결 성공 직후 1회 호출되는 `flush` 가 빈 큐를 보고 즉시 끝난 뒤 등록된 항목은 그 연결에서 영영 소비되지 않는다 | `partial:b2-interaction` (post-round) |

### 7건을 7줄로 쓰지 않고 한 열로 묶은 이유

분류기의 처방 문장은 7개 전부 「b1(또는 신설 축)에 …를 추가해야 한다」였고, 그대로 받으면 조건부
열이 3개에서 10개가 된다. 그런데 7건의 **조건이 하나**다 — *비동기 판정이 진행 중인데 그 입력이
바뀔 수 있다.* 조건이 같으면 열이 하나여야 한다. #59 가 세 열을 각각 세운 이유가 「세 열 중 어느
것도 모든 surface 에 해당하지 않는다」였고, 여기서는 그 반대 사정이 성립한다.

7건의 차이는 **그 칸에 무엇을 대입하는가**이고, 셋으로 갈린다.

| 대입 | GT | 묻는 것 |
|---|---|---|
| 폐기된 실행이 남기는 것 | 1·6 | 시작 입력 ≠ 최신 입력인 실행이 마커·부수효과를 그대로 내보내는가 |
| 최신 입력의 재판정이 도는지 | 3·4 | 그 경로를 다시 깨우는 것이 무엇인가 — ref 쓰기와 동일 값 setState 는 렌더를 만들지 않는다 |
| 가드와 주체의 수 | 5·7·9 | 가드 기준값의 갱신 시점, 중첩 실행의 세대·단일화, 주체가 둘일 때의 승자·패자 칸 |

셋을 각각 열로 세우지 않은 이유는 **한 칸을 채우려면 셋이 다 필요하다**는 것이다. GT 1 이 성립
하려면 폐기 판정이 있어야 하고(가드), 그 판정이 남긴 마커가 재판정을 막아야 한다(재판정). 열을
쪼개면 같은 실행을 세 칸에 나눠 적게 되고, 어느 칸도 「사용자가 무엇을 읽는가」에 도달하지 않는다.

또 하나 — **이 계열은 「가드를 넣었다」로 닫히지 않는다.** ceo-client#874 는 같은 자리를 4라운드
연속 고쳤고, `d69c159f6` 이 조건부 `settledPathRef` 가드를 「뮤테이션이 실패하지 않았으니 지키는
것이 없다」로 **의도적으로 기각**했는데 두 커밋 뒤 `83fb36743` 이 그것을 load-bearing 으로 되돌렸다
(`isLoading` 가드가 들어오면서 조합이 도달 가능해졌다). 그래서 열의 문장을 「가드가 있는지」가
아니라 「그 가드의 기준값이 언제 갱신되는지 대조한다」로 썼다.

### a-code 로 보낸 GT 2 — 자리를 「입구」 문단으로 고른 이유

GT 2 는 화면 칸이 아니다. 어긋난 조합이 없고, 사용자가 읽는 것이 틀리지도 않는다 — 같은 요청이
여러 번 나가고 끝난 판정이 다시 도는 **코드 정합성**이다. 실측이 그것을 말한다: `021d92749` 의
커밋 메시지가 「실측 결과 `authenticate` 실행이 동시에 17개 돌고 있었다」다.

a-code 안의 자리 후보는 셋이었다.

- **「입구」 문단 (선택)** — 그 문단이 이미 열거 대상에 **의존성 배열**을 넣어 뒀고(`무효화 세대 키`
  옆), 묻는 것이 「통과시키는 집합이 의도한 집합인가」다. 의존성 배열에서 그 집합은 **재실행을
  트리거하는 값의 집합**이고, 원소 하나가 렌더마다 새 참조면 그 집합은 「값이 바뀔 때」가 아니라
  「매 렌더」다 — 기존 문장의 **넓어진 쪽**이 그대로 적용된다. 새 절을 세우지 않고 한 문장을 얹었다.
- 실패 경로의 **수명** 문단 (기각) — #60 이 넣은 그 문단은 실패·이탈 이후를 본다. GT 2 는 정상
  경로에서 성립한다.
- 1번 **새 회귀** (기각) — pre-existing 이다. `useUser` 가 렌더마다 새 `queryKey`/`queryFn` 을
  만드는 것은 이 PR 이 만든 것이 아니고, 이 PR 이 그 위에 effect 를 얹어 도달 가능하게 했다.

### #60 「수명」·#61 「두 번 발화하는가」와 무엇이 다른가

세 자리가 모두 「비동기 작업이 여러 번 도는 것」처럼 읽히는 자리다. 경계는 **무엇이 그것을 다시
시작시키는가**다.

| 자리 | 트리거 | 산출물 |
|---|---|---|
| `b2` 「두 번 발화하는가」 — issue #61 | **사용자가 in-flight 중 다시 누른다** | 그 컨트롤에서 요청·이벤트가 몇 번 나가는지, 버튼이 실제로 `disabled` 로 렌더되는지 |
| `a-code` 실패 경로 「수명」 — issue #60 | **호출부가 먼저 진행한다**(타임아웃·폴백·이탈) | 그 뒤에도 *살아 있는* 요청·타이머·구독·워커의 목록과 점유 시간 |
| `a-code` 「입구」 의존성 배열 — issue #52 | **렌더가 일어난다** | 렌더마다 새 참조가 되는 의존성 원소의 목록과, *끝난* 작업이 다시 시작되는 횟수 |

프롬프트 본문에는 「수명」과의 차이만 한 괄호로 적었다(*살아 있는* / *끝난 작업이 다시 도는*).
`b2` 와의 차이는 축이 달라 한 리뷰어가 두 문장을 동시에 읽지 않으므로 본문에 쓰지 않았다 — 축
경계는 이 문서의 몫이다.

### b2 GT 10 은 넣었다 — 근거 등급이 post-round 1건인데도

**판단: 넣는다.** 근거는 셋이다.

1. **자리가 이미 있다.** #61 이 자동 경로 열거 대상에 「연결 성공·재연결 직후 1회 실행되는 소비
   로직」을 넣어 뒀다. 그 surface 는 이미 목록에 오르는데 **등록-소비 순서**를 묻지 않아서
   「1회 실행된다」까지만 확인하고 끝난다. 추가분이 새 절도 새 열도 아니라 그 문단의 한 문장이다.
2. **비용이 근거 등급에 비례한다.** post-GO 가 아니라 post-round 1건이므로 새 자리를 만들지
   않는 것이 맞다 — #63 이 pre-gate 1건을 아예 넣지 않은 것과 #59 가 pre-gate 1건을 열거 대상
   근거로만 쓴 것 사이가 이 경우다.
3. **조건이 코드에서 관측된다.** 「일회성 이벤트에 붙은 소비 로직인가」는 리스너 등록 지점과 그
   핸들러가 큐·맵을 읽는지로 확인된다. 사람의 판단을 요구하지 않으므로 무음으로 생략되지 않는다.

기각한 대안은 `a-code` 였다(분류기 처방이 「b2-interaction 또는 a-code」였다). GT 10 의 도착점이
**사용자가 보는 것이 바뀌지 않는 것**이다 — 실패한 메시지 버블이 세션 내내 자동 재전송되지 않는다.
b2 의 「해소한다의 기준은 사용자가 보는 것이 바뀌는 것이다」가 그 판정이고, a-code 에 두면 「등록이
소비보다 늦다」는 순서 지적으로 끝나 사용자가 무엇을 보는지에 도달하지 않는다.

### 4축 중복 검토 — 현재 파일 전수 대조

`b1`·`b2`·`a-code`·`b4` 를 전문 읽고 새 문장과 대조했다. 겹쳐 보이는 자리 4개의 경계다.

| 기존 문장 | 새 문장 | 경계 |
|---|---|---|
| `b1` 「`useEffect` 로 복사하는 store 는 렌더 이후에 갱신된다」(#59) | 「가드 기준값이 갱신되는 시점(render / layout effect / passive effect)을 가드 평가 시점과 대조」(GT 7) | 앞은 **두 데이터 소스**가 어긋나는 렌더 수를 센다(판정 소스 ≠ 렌더 소스). 뒤는 **가드 하나**가 읽는 기준값이 가드 평가보다 늦게 갱신되는지를 본다 — 소스가 하나이고 대조 대상이 시점이다 |
| `b2` 「이탈한 뒤 도착하는 응답 … 취소 의도가 클릭 시점에 기록되는지」(#61) | 「부수효과(라우팅·쿠키 쓰기)를 적용하기 직전에 그 입력이 아직 최신인지 확인하는 코드가 있는지」(GT 6) | 앞은 **컨트롤**(누르는 것·popstate)에서 출발해 사용자의 이탈·취소 의도를 본다. 뒤는 컨트롤이 없는 자리에서도 성립한다 — GT 6 의 AuthGuard 는 경로 변경으로 스스로 도는 판정이고, 묻는 것은 취소 의도가 아니라 **판정 입력의 신선도**다. 새 문장에서 전역 UI(스낵바·토스트)는 뺐다 — 그쪽은 b2 소관이고 GT 8 이 이미 덮였다 |
| `b2` 「두 번 발화하는가」(#61) | a-code 의존성 배열 문장(GT 2) | 위 §#60 「수명」·#61 「두 번 발화하는가」와 무엇이 다른가 표. 트리거가 재클릭 / 렌더로 갈린다 |
| `b4` 「값이 쓰이는 순서도 본다 — 상태 삭제·초기화가 같은 값의 이후 읽기보다 먼저 실행되는지」(#62) | 「최신 입력의 재판정이 도는지」(GT 3·4) | 앞은 한 렌더 안 **동기 실행 순서**에서 nullable 이 기본값으로 하류에 나가는지를 본다. 뒤는 **렌더가 한 번 더 일어나는지**를 본다 — 값의 유무가 아니라 렌더 발생 여부이고, 산출물이 「그 경로를 무엇이 깨우는가」다 |

`a-code` 의 「형제·소비처·입구·범위·수명」과 `b1` 의 「조건부 열 3개·분기 키 축·판정소스≠렌더소스」,
`b4` 의 「기존 nullable·쓰기 순서·truthy 빈 값」에는 새 문장과 문자열이 겹치는 자리가 없다.

### 손 대조 5건 (원본 커밋)

새 지시가 실제 결함의 표면을 열거하는지 **결함이 존재했던 커밋**에서 확인했다. ceo-client#874 는
같은 파일을 4라운드 연속 고쳤으므로 GT 마다 결함 커밋이 다르다.

| GT | 결함 커밋 → 정정 커밋 | 대조 결과 |
|---|---|---|
| 1 (폐기된 실행이 마커를 기록) | `b19e2f3b6` → `83fb36743` | 결함 커밋 `apps/TalkWeb/components/AuthGuard.tsx:279-281` 의 `.finally()` 가 `isRunningRef.current = false` 다음 줄에서 `settledPathRef.current = pathAtStart` 를 **무조건** 쓴다. 같은 파일 `:197` 이 `const isStale = () => pathAtStart !== latestPathRef.current` 이고 `:200`·`:216`·`:237` 이 그것으로 부수효과와 `setIsLoading(false)` 를 건너뛰는데, **마커 쓰기만 그 가드 밖에 있다.** 결함 커밋 자신의 주석 `:191-195` 가 「버린 실행도 `settledPathRef` 는 시작 경로로 세워진다 … 표시는 사용자가 시작 경로로 되돌아오기 전에 반드시 교체된다」로 안전을 **단정**한다. 정정 커밋이 `if (pathAtStart === latestPathRef.current)` 로 감싸며 「A 판정 pending → B 로 이동 → A 실행이 버려짐(해제 안 함) → settleTick effect 가 돌기 전에 뒤로가기로 A 복귀 → `settledPathRef === A` 가드가 즉시 반환 → 새 실행도 폴백도 없고 `isLoading` 이 true 로 남아 화면이 통째로 백지」를 남겼다. 「완료·중복방지 마커(ref·플래그)를 그 실행도 기록하는지 적는다」가 지목하는 대조가 `:237`(가드 안) ↔ `:281`(가드 밖)이다 |
| 7 (가드 기준값의 갱신 시점) | `b19e2f3b6` → `83fb36743` | 같은 결함 커밋 `:56-58` 이 `useEffect(() => { latestPathRef.current = currentPath }, [currentPath])` 다 — **passive effect** 다. 그 ref 를 읽는 것이 `:197` 의 `isStale()` 이고, 그것을 평가하는 자리는 `:200`(성공)·`:216`(실패)·`:237`(finally) 로 전부 await 이 resolve 한 **마이크로태스크** 안이다. 정정 커밋이 `const useCommitEffect = typeof window === "undefined" ? useEffect : useLayoutEffect` 로 바꾸며 「React 는 commit 과 passive effect flush 사이에 마이크로태스크를 흘려보내므로, 경로가 commit 된 직후 pending 이던 인증 Promise 가 그 틈에 완료되면 아직 옛 경로가 담긴 ref 를 읽어 stale 판정을 최신 경로에 적용한다」를 남겼다. 커밋 메시지가 이 건을 「pre-existing; independently reported by a PR reviewer」로 적고 「`act()` 가 commit 과 effect flush 를 합치므로 jsdom 에서 재현할 수 없다 — 뮤테이션으로도 확인되지 않는다」고 남긴다. **테스트로 잡히지 않는 자리**라 열거 지시가 유일한 포획 수단이라는 것이 이 행의 내용이다 |
| 3·4 (재판정 깨우기 · 동일 값 렌더 생략) | `33503eab5` → `36590256b` | 결함 커밋 `AuthGuard.tsx:237-240` 의 `.finally()` 가 `isRunningRef.current = false`·`settledPathRef.current = pathAtStart` **두 ref 쓰기만** 하고, effect 의 deps 는 `:241` `[authenticate, authError, currentPath]` 다. 그 위 세 가드가 `:202` `authError`·`:207` `isRunningRef.current`·`:220` `settledPathRef.current === currentPath` 이므로, 실행 중 경로가 바뀌면 새 경로 effect 는 `:207` 에서 반환하고 이후 그 effect 를 다시 돌릴 계기가 없다. `:195-197` 의 `finally` 는 `setIsLoading(false)` 를 하지만 최초 판정 이후에는 **동일 값**이다. 정정 커밋이 `settleTick` 카운터를 세우고 deps 에 넣으며 「이 `finally` 는 ref 만 바꾸므로 리렌더가 없다 — `setAuthError(null)`·`setIsLoading(false)` 는 최초 판정 이후 둘 다 **동일 값** 업데이트라 후속 렌더를 보장하지 않는다 … 그 경로는 이 컴포넌트가 살아 있는 동안 영구히 미판정으로 남는다」를 남겼다. 「완료를 알리는 것이 ref 쓰기뿐이면 리렌더가 없고, 상태 갱신이 동일 값이면 React 가 렌더를 생략한다. 그 경로가 재판정을 무엇으로 깨우는지 이름으로 적는다」가 지목하는 대조가 `:237-240`(무엇을 쓰는가) ↔ `:241`(무엇이 deps 인가)이다 |
| 5·6 (중첩 복구 순서 역전 · stale 결과의 부수효과) | `36590256b` → `d69c159f6` | 정정 커밋 메시지가 두 건을 나란히 적는다. GT 6: 「`runAuthenticate` 가 `currentPath` 를 클로저로 캡처해 `redirectTo` 를 정한다. 실행이 사용자가 이동한 뒤 끝나면 그 판정이 그대로 적용됐다 — `/` 판정이 `router.replace("/chat")` 를 발화해 방금 연 화면에서 끌어내고, 30일 비밀번호 확인 쿠키를 쓰고, 이전 경로의 실패가 현재 화면의 폴백이 됐다. `36590256` 의 `settleTick` 은 새 경로를 재판정하지만 **이미 나간 부수효과를 되돌리지 못한다.**」 GT 5: 「`onSessionError` 가 `connectWithFetchUser()` 를 await 없이 호출하고 SDK 는 한 번의 장애에 그것을 반복 발화할 수 있다. 각 호출이 자기 `fetchNewUserToken` → `sdk.connect` 를 돌려 **완료 순서로 마지막 쓰기가 이긴다** — 늦은 실패가 성공의 `connectionLost: false` 를 덮어 살아 있는 연결 위에 폴백을 고정하거나, 반대로 죽은 연결을 정상으로 표시했다.」 정정은 in-flight 복구 Promise 재사용(단일화)이다. 「부수효과를 적용하기 직전에 그 입력이 아직 최신인지 확인하는 코드가 있는지」와 「세대·단일화가 없으면 중첩 실행 중 늦게 끝난 이전 실행이 최신 결과를 덮는 조합을 적는다」가 각각 지목하는 자리다 |
| 9 (승자·패자 · 세대) | `8553c943a` → `4cb8560d5` (ceo-client#884) | 정정 커밋 메시지: 「소유권은 connect 호출이 반환할 때 기록됐지만 앱은 SDK 의 connect 를 await 하지 않는다 — UIKit 래퍼를 await 하고, 그 래퍼는 `sdk.connect` 가 resolve 한 뒤에도 `updateCurrentUserInfo`·푸시 등록·`initEmoji`·`initDashboardConfigs` 를 계속한다. **연결이 성립하는 순서와 producer 가 소유권 주장에 도달하는 순서가 같은 순서가 아니다.** 그 틈이 오래된 세대의 producer 가 늦게 끝나 이미 연결된 새 세대의 주장을 덮어쓰고, 자신을 소유자로 알고 살아 있는 연결을 끊게 했다 — 그 주장이 막으려던 바로 그것이다.」 정정은 주장에 세대를 실어 새 세대의 것을 밀어내지 못하게 하고, 해제는 토큰이 아직 유효할 때만 하는 것이다. 같은 PR `1c88e9452` 가 화면 진입에서 같은 모양을 「producer 가 둘(자동 진입 effect·`onLoginComplete`)이면 어떤 판정 규칙도 안전하게 만들지 못한다 — 진입은 한 번 시작하고 먼저 시작한 쪽이 이긴다」로 닫았다. 「같은 자원을 동시에 요청하는 주체가 둘 이상이면 승자·패자 각각에 칸을 따로 채운다」가 지목하는 두 주체가 이것이고, GT 9 의 「패자만 화면을 갱신하는 조합」이 그 칸 하나다 |

**`red-team/SKILL.md` 는 건드리지 않았다.** 바뀐 것은 `b1-state-matrix.md`(+13줄)·`a-code.md`
(+4줄)·`b2-interaction.md`(+4줄)와 `references/design.md`·이 문서뿐이라 예산 검사 대상이 아니다.

## replay 배선 — 과거 라운드를 같은 조건으로 다시 돌린다

프롬프트·정책 변경의 효과를 재려면 **입력을 고정하고 그것만 바꿔야** 한다. 과거 라운드의
`context.md`·`diff.md` 가 `runs2/<repo키>/<branch키>/<gate>-<n>/` 에 그대로 남아 있으므로
재현이 가능하다. 2026-09-09~10 측정에서 쓴 절차다.

1. **워크트리** — `git worktree add --detach <path> <그 라운드가 리뷰한 커밋>`. 커밋은 그
   라운드 `context.md` 의 `## 리뷰 대상` 절에 `<base>..<head>` 로 적혀 있다.
2. **`node_modules` 를 그 브랜치 lockfile 로 설치** — `yarn install --immutable`. **이걸
   빠뜨리면 실험이 무효가 된다**: 현재 master 의 사본을 심볼릭 링크하면 `a-code` 3번의 정당한
   지시(「최근접 `node_modules` 사본이 최종 권위」)를 따라 **오늘의 의존성에는 참이고 리뷰 대상
   시점에는 거짓인** finding 이 나온다. 실측에서 1차 실험의 finding 2건이 전부 그렇게 생긴
   산물이었다(`@sendbird/chat` 4.19.2 를 4.22.9 로, account SDK 1.7.30 을 1.7.44 로 인용).
   기존 심볼릭 링크는 **먼저 지운다** — 안 지우면 원본 저장소의 `node_modules` 에 쓴다.
   워크스페이스 모노레포는 스코프 패키지가 `apps/<app>/node_modules/@scope/name` 에 깔리고,
   lockfile 에 같은 패키지의 여러 버전이 있을 수 있으므로 **그 워크스페이스의 `package.json`
   선언**으로 기대 버전을 정한다.
3. **재현 검증** — `git diff <base>...HEAD` 의 바이트 수·파일 수가 저장된 `diff.md` 와 맞는지
   확인하고 시작한다.
4. **diff 부분집합이 필요하면** 합성 base 커밋을 쓴다 — `base → X(다른 그룹만 적용) → H(전체,
   트리는 원본과 동일)` 를 만들고 `--diff-base X`. 러너는 `git diff <base>...HEAD`(3-dot)라
   base 가 HEAD 의 조상이어야 부분집합이 나온다. 트리 동일성은 `git diff --stat H <원본커밋>`
   이 비는 것으로 검증한다.

**ground truth 를 고를 때 봉인을 먼저 거른다.** 그 라운드 `context.md` 의 `## 스코프 밖`·
`## 이미 반영된 지적` 에 등재된 주제는 리뷰어가 올릴 수 없으므로, 봉인된 항목을 GT 에 넣으면
설계 효과가 아니라 봉인 여부를 재게 된다. 봉인을 해제한 컨텍스트로 돌리거나 그 항목을 GT 에서
뺀다.

**판정 기준은 라운드를 돌리기 전에 적는다.** 결과를 보고 기준을 만들면 그 기준이 원하는 결론에
맞춰 조정된 것이라 근거가 못 된다.

**샘플 수.** 같은 입력 3샘플이 서로 다른 finding 집합을 냈다(고유 4개, 겹친 것 1개, 한 샘플 최대
2개). 1샘플로 축의 발화·침묵을 효과라고 부르면 안 된다 — 측정 결과는
`references/design.md` 의 「15개 PR 보강 후 실측」 절.

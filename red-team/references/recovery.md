...
**본문은 `assets/aborted-template.md` 를 복사해 채운다** — `## 사유` / `## 미해결 P1` /
`## 재개 조건` / `## 식별 정보` 네 절이다.

- 본문에 **저장소 원격 URL 과 원본 브랜치명을 함께 적는 것을 권장**한다 — 키가 같아도 워크트리는
  여러 개일 수 있고, 안내에 표시되는 본문으로 어느 작업의 중단인지
  즉시 식별되는 가치가 있다(템플릿의 `## 식별 정보` 절이 그 자리다).
- **`## 미해결 P1` 을 비우지 않는다.** 나간 채로 남은 결함 목록이 사유와 함께 있어야
  `pr-triage` 가 리뷰어 지적을 받았을 때 "이건 이미 알고 나간 것" 으로 가른다
  (소비 절차: `pr-triage/references/edge-cases.md`). 정말 없으면 `없음` 이라고 적는다 —
  빈 절은 "확인했고 없다" 와 "확인하지 않았다" 가 구분되지 않는다.
  이 목록은 **사람·`pr-triage` 용 본문이고 판정 입력이 아니다** — 중단 상태의 판정은
  파일의 존재 하나로 끝난다(산문 파싱 금지 불변식은 그대로다).
...
## 교착이면 ABORTED 를 먼저 제시한다 — 라운드를 더 권하지 않는다 (issue #65)

**발동**: 남은 P1 이 **전부 코드 밖 판단**(기획·PO·타 팀 결정)을 기다려 이 브랜치의 코드로는
닫을 수 없는 상태. 수렴 실패가 아니라 교착이다.

실측(근거: `evidence.md` 「NO-GO 교착에서 사람이 고른 것은 '그냥 나가기' 였다」)에서 이
상태의 결말은 ABORTED 가 아니라 **"그냥 나가기"** 였다 — 기록에 남는 마지막 상태가 NO-GO 라서
나중에 그 티켓을 이어받거나 집계할 때 **수렴 실패와 정당한 중단이 구분되지 않는다.**

`resume.py` 가 두 자리에서 중단 종결을 **함께 제시한다**(차단하지 않는다):

| 자리 | 왜 그 자리인가 |
|---|---|
| `보류` 절이 비지 않아 다음 라운드가 막혔을 때 | 그 자체로 "지금 코드로 닫을 수 없는 것이 있다" 는 신호다. 차단 문구만 주면 출구가 `결정` 하나로 보인다 |
| `verdict=NO-GO` 이고 **라운드 번호가 3 이상**일 때 | 탈출 결함이 나온 5건의 코드 라운드 수가 3·5·5·13·17 이었다. 임계는 `long-gate.md` 진입 조건과 같은 3이다 |

제시를 받으면 사람이 셋 중 하나를 고른다 — **① 라운드를 더 돈다**(코드로 닫을 여지가
남았다) / **② 코드 밖 판단을 먼저 받아온다**(그때까지 라운드를 세우지 않는다) /
**③ `ABORTED` 로 닫는다**(미해결 P1 을 본문에 남긴다). `long-gate.md` §0 의 same-origin
항목이 가리키는 "생존성 7·8행 재심 → 중단/피벗" 과 같은 출구다.

### 왜 자동 판정이 아니라 제시인가 — 열린 질문의 결정

**결정: 사람에게 묻는다(제시만 한다). 자동 교착 판정·자동 조기 종료를 만들지 않는다.**

기계가 볼 수 있는 것은 `보류` 가 비지 않았다는 사실과 라운드 수뿐이고, **그 둘은 수렴
실패에서도 똑같은 값이다.** "남은 P1 이 코드 밖 판단을 기다린다" 는 것 자체가 산문이라
기계는 그 구분을 원리적으로 못 한다. 게다가 오탐 비용이 비대칭이다 — 자동 종료의 오탐은
결함을 안 채로 게이트를 닫고, 제시의 오탐은 안내 한 줄이다. 그래서 신호는 기계가 내고
판정은 사람이 한다.

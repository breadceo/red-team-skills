...
- **PRD·Architecture·Spec 단계에 계획 게이트를 걸지 않는다.** `a-plan` 은 *구현 계획* 리뷰어다.
  ...
  그건 sub-PR 리뷰 대상이라 값이 있다. SKILL.md 의 「계획 문서를 쓰나」 판별표는 *구현 계획*
  에만 적용되므로 이 세 단계는 그 표 밖이다 — 판별표가 이 규정을 대체하지 않는다.
  반대로 위 흐름의 ① `--gate plan` 은 무조건이다: `/task plan` 이 이미 구현 계획 문서를
  만들었으므로 판별표의 「계획 문서가 이미 있으면 판별 없이 탄다」에 걸린다.

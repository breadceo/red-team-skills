...
def main():
    ...
    assert body.index("## 이 변경이 하려는 것") < body.index("## 판정 기준") < body.index("## 스코프 밖"), \
        "판정 기준 절 위치가 어긋났다"
    # 계획 게이트 초안에는 문서 6-구조 + 생존성 커버리지 표가 빈 채로 깔린다 (채우는 것은 clarify 단계)
    assert "## 판정 기준 — 문서 6-구조 + 계획 생존성 커버리지" in body, "확장 제목의 커버리지 표가 계획 초안에 없다"
    assert body.count("<반영/해당없음/누락>") == 8, "커버리지 행이 8개(문서 6 + 생존성 2)가 아니다"
    assert "신뢰 경계" in body and "과도기 규정" in body, "6-구조 행 내용이 빠졌다"
    assert "설계 수명·로드맵 충돌" in body and "통증 위치 대조" in body, "생존성 7·8행이 빠졌다"
